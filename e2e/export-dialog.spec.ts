...
test.describe('Export Dialog', () => {
  ...
  test('PNG format shows Regular/High quality toggle instead of slider', async ({ page }) => {
    await page.getByRole('button', { name: 'File' }).click();
    await page.waitForTimeout(150);
    await page.getByRole('menuitem', { name: 'Export…' }).click();
    await page.waitForTimeout(200);
    ...
    // PNG is the default format — shows Regular/High toggle, not a slider
  ...
  test('Export button triggers file download', async ({ page }) => {
    ...
    // Default format is PNG, filename defaults to 'lopsy'
