...
test.describe('Export Dialog', () => {
  ...
  test('quality range slider is hidden when PNG is selected', async ({ page }) => {
    await page.getByRole('button', { name: 'File' }).click();
    await page.waitForTimeout(150);
    await page.getByRole('menuitem', { name: 'Export…' }).click();
    await page.waitForTimeout(200);
    ...
    // PNG is the default format — lossy quality range slider should not be present
    // (PNG shows a Regular/High toggle instead)
  ...
  test('Export button triggers file download', async ({ page }) => {
    ...
    // Default format is PNG, filename defaults to document name or 'lopsy'
