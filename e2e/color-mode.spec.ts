...
test.describe('Document color mode', () => {
  ...
  test('painting in CMYK keeps the painted color instead of going black', async ({ page }) => {
    await page.goto('/');
    await waitForStore(page);
    await createDocument(page, 200, 200, false);
    await page.waitForSelector('[data-testid="canvas-container"]');
    ...
    // Regression: CMYK once stored black ink in the alpha channel, but the
    // paint pipeline owns alpha — it writes coverage there and premultiplies by
    // it. Every stroke came out solid black with a thin colored fringe.
    await drawRect(page, 40, 40, 90, 90, { r: 220, g: 40, b: 40 });

    const px = await readCompositedAtDoc(page, 85, 85);
    expect(px.r).toBeGreaterThan(180);
    expect(px.g).toBeLessThan(90);
    expect(px.b).toBeLessThan(90);
  });

  test('CMYK keeps its layer stack and round-trips back to RGB unchanged', async ({ page }) => {
    await page.goto('/');
    await waitForStore(page);
    await createDocument(page, 160, 160, false);
    await page.waitForSelector('[data-testid="canvas-container"]');

    await drawRect(page, 30, 30, 90, 90, { r: 60, g: 140, b: 200 });
    const before = await readCompositedAtDoc(page, 70, 70);

    await selectColorMode(page, 'CMYK Color');

    // Alpha is real alpha again, so layers work exactly as in RGB.
    const layersBefore = (await getEditorState(page)).document.layers.length;
    await addLayer(page);
    expect((await getEditorState(page)).document.layers.length).toBeGreaterThan(layersBefore);

    // The naive ink model is a bijection, so the round trip is lossless.
    await selectColorMode(page, 'RGB Color');
    ...
    expect(Math.abs(after.b - before.b)).toBeLessThanOrEqual(2);
  });
