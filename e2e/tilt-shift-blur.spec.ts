...
test.describe('Tilt-Shift Blur Filter', () => {
  ...
  test('blurs edges while preserving sharp stripes in focus band', async ({ page }) => {
    ...
    // Focus Position = 50 (center) — nth(0)
    // Focus Width = 15 (narrow focus band) — nth(1)
    await setRangeValue(sliders.nth(1), '15');
    await page.waitForTimeout(100);
    // Blur Radius = 25 (strong blur for dramatic effect) — nth(2)
    await setRangeValue(sliders.nth(2), '25');
    await page.waitForTimeout(200);
  ...
  test('undo restores original pixels after tilt-shift blur', async ({ page }) => {
    ...
    const sliders = dialog2.locator('input[type="range"]');
    await setRangeValue(sliders.nth(1), '10');
    await page.waitForTimeout(100);
    await setRangeValue(sliders.nth(2), '30');
    await page.waitForTimeout(200);
  ...
  test('dialog UI shows all controls and cancel works', async ({ page }) => {
    ...
    const dialogHeading = page.locator('[role="dialog"][aria-label="Tilt-Shift Blur"]');
