...
test.describe('Align layer content', () => {
  test('align left moves content to left edge', async ({ page }) => {
    ...
    await drawRect(page, 50, 50, 20, 20, { r: 255, g: 0, b: 0 });
    ...
    expect(Math.abs(pos.y - 50)).toBeLessThanOrEqual(1);
  });
  ...
  test('align right moves content to right edge', async ({ page }) => {
    ...
    // ±1px tolerance: shape tool mouse events introduce rounding in screen-to-doc conversion.
    expect(Math.abs(pos.x - 180)).toBeLessThanOrEqual(1);
    expect(Math.abs(pos.y - 50)).toBeLessThanOrEqual(1);
  });
  ...
  test('align top moves content to top edge', async ({ page }) => {
    ...
    expect(Math.abs(pos.y - 0)).toBeLessThanOrEqual(1);
  });
  ...
  test('align bottom moves content to bottom edge', async ({ page }) => {
    ...
    // ±1px tolerance: shape tool mouse events introduce rounding in screen-to-doc conversion.
    expect(Math.abs(pos.x - 50)).toBeLessThanOrEqual(1);
    expect(Math.abs(pos.y - 180)).toBeLessThanOrEqual(1);
  });
  ...
  test('align center horizontally', async ({ page }) => {
    ...
    // ±1px tolerance: shape tool mouse events introduce rounding in screen-to-doc conversion.
    expect(Math.abs(pos.x - 90)).toBeLessThanOrEqual(1);
    expect(Math.abs(pos.y - 50)).toBeLessThanOrEqual(1);
  });
  ...
  test('align with full-layer content', async ({ page }) => {
    ...
    await drawRect(page, 0, 0, 200, 200, { r: 255, g: 255, b: 0 });
  ...
  test('sequential alignments compose correctly', async ({ page }) => {
    await drawRect(page, 80, 80, 20, 20, { r: 255, g: 0, b: 255 });
  ...
  test('align to bottom-right corner', async ({ page }) => {
    ...
    // ±1px tolerance: shape tool mouse events introduce rounding in screen-to-doc conversion.
    expect(Math.abs(pos.x - 170)).toBeLessThanOrEqual(1);
    expect(Math.abs(pos.y - 170)).toBeLessThanOrEqual(1);
  });

  test('center both axes', async ({ page }) => {
    await drawRect(page, 0, 0, 40, 60, { r: 128, g: 128, b: 128 });
    ...
    // ±1px tolerance: shape tool mouse events introduce rounding in screen-to-doc conversion.
    expect(Math.abs(pos.x - 80)).toBeLessThanOrEqual(1);
    expect(Math.abs(pos.y - 70)).toBeLessThanOrEqual(1);
  });
});
