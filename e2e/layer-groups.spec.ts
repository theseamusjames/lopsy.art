...
test.describe('Visual rendering after group move', () => {
  test('canvas shows content after layer moved out of hidden group', async ({ page, isMobile }) => {
    test.skip(isMobile, 'layer panel requires sidebar, hidden on touch devices');
    await page.goto('/');
    await waitForStore(page);
...
test.describe('GPU rendering verification', () => {
  test('layer content renders after being moved out of hidden group', async ({ page, isMobile }) => {
    test.skip(isMobile, 'layer panel requires sidebar, hidden on touch devices');
    await page.goto('/');
    await waitForStore(page);
    await createDocument(page, 100, 100);
