...
// Levels: 256x1 RGBA texture, packed the same way as the curves LUT
// (R/G/B = per-channel, A = master). Applied BEFORE curves so the
// classic "levels then curves" workflow matches what users expect.
...
void main() {
    ...
    // Applied before curves so the two adjustments compose in the
    // expected "tonal re-map → curve shape" order.
    if (u_hasLevels > 0.5) {
        float ofs = 0.5 / 256.0;
