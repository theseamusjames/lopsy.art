...
- **Whichever route it takes, the click has already paid a full-layer GPU→CPU readback before the route is chosen.** Fill is in the `needsPixelData` set, so `expandLayerForEditing` runs at pointer-down, ahead of any tool handler: it materializes the layer's pixels JS-side — reading the texture back when there is no cache, which is the normal case — and reconciles `layer.x/y/width/height` against them. The reconciliation is not optional. The GPU fill entry points call the engine's `ensure_layer_full_size`, which re-origins the engine's layer descriptor without telling the store, so dropping the expand (#722, briefly on main inside #726) made a fill on any content-offset layer — a moved layer, or a content-cropped one — composite at *double* its offset; #735 restored it. The fill drops the JS cache again on the way out, so the readback repeats on every click (~67 MB on a 4096 × 4096 layer).
- **One consequence: the empty-layer route is currently unreachable from a canvas click.** It requires the layer to have *no* JS pixel data as well as a 1 × 1 lazy GPU texture, and the expand above always leaves dense JS data behind. A blank layer is therefore filled by the contiguous CPU flood — or by the color-match shader with Contiguous off — which produces identical pixels, so this shows up as time rather than as a wrong result.
