...
- Holding the cursor still keeps emitting dots on a 166 ms timer (~6 Hz) so paint accumulates over time, mimicking an airbrush. Dragging emits a fresh cloud each time the pointer has travelled `max(1, 0.3 × Size)` px, so consecutive clouds overlap by about 70% of their width at every Size.
  - **Slow drags spray, and the timer follows the cursor (#793, fixed in #808).** Two bugs in the same handler. A move shorter than the spacing used to advance the anchor point *without* emitting, throwing the travelled distance away — so a natural-speed drag made of many small pointer events laid down nothing after the pointer-down cloud. The anchor now stays pinned until the accumulated distance crosses the spacing. And the 166 ms hold timer closed over the pointer-down state object, which the interaction dispatcher shallow-copies every frame, so the "airbrush" kept spraying at the **pointer-down point** no matter where the cursor went. The live cursor position is now tracked at module scope and the timer reads that.
...
**Known defect — four tools push two undo entries per stroke.** The shared paint-tool path at pointer-down pushes a history entry labeled `Brush`, `Pencil`, or `Spray` for those three tools and **`Eraser` for every other paint tool** — and Dodge / Burn, Sponge, Clone Stamp, and Healing Brush each then push their own correctly-labeled entry from their own handler. `pushHistory` does not coalesce, so one stroke with any of those four leaves two rows in the History panel: a spurious **"Eraser"** followed by the real one, and undoing the stroke takes two steps. Spray was a fifth until #791 removed its own push (see [Spray](#spray)). (Smudge is unaffected — it is not registered as a paint tool, which is also why it has no straight-line origin. And the defect vanishes in layer-mask edit mode and Quick Mask, because the shared setup that pushes the spurious row is skipped whenever a mask mode is on — though there these tools paint the layer instead of the mask; see [Editing a Layer Mask](#editing-a-layer-mask).)
...
- **Shape types**: ellipse, polygon. A rectangle/square is drawn as a
  ...
  - **Sides = 4 is special-cased into a true rectangle (#794, fixed in #808).** Every other side count goes through `sdPolygon`, which fits a *regular* polygon to `min(w, h)` — so until #808 a 4-sided drag drew a centred **square** in Pixels output and a stretched **diamond** (vertices at top/right/bottom/left) in Path output, and there was no way to draw a non-square rectangle at all. `shape_fill.glsl` now routes `u_sides == 4` to `sdRect` at the full drag aspect (honouring Corner radius), and `polygonToPathAnchors` short-circuits n = 4 to the four bounding-box corners, so both outputs give the same axis-aligned rectangle. Other side counts still fit a regular polygon inside the drag box.
...
- The selection mask is not recomputed during the drag. It is rebuilt only
  ...
  buttons when no transform is pending, and the selection-branch **Rotate 90°**
  buttons always, which re-select after every click (see
  [Quick transforms](#quick-transforms)). **`Delete` left this list in #808
  (#801)** — it now drops the float and clears with the marquee as it stands;
  see [Clipboard](#clipboard). The swap pushes no history row of its own, so no single undo
...
- **Flip Horizontal / Flip Vertical** (options bar, next to the mode buttons)
  ...
  - **With a scale or rotate still pending, the flip composes into it instead
    (#800, fixed in #808).** When the mode is **Free**, the UI transform is not
    the identity, and the engine holds a float, the button negates `scaleX`
    (horizontal) or `scaleY` (vertical) of the pending transform and
    re-renders the float through `compositeFloatAffine` — no drop, no
    re-select, no history row — so the handles stay live and `Enter` / `⌘D`
    commits the flipped result. Until #808 the click took the commit path above
    and silently threw the pending scale and rotation away. In Skew, Distort,
    or Perspective mode, or with nothing pending, the old commit-and-reselect
    path still runs.
...
- **Alt/Option+drag (no active marquee)**: duplicates the active layer in place, then moves the new copy — leaves the original layer untouched. The duplicate is deliberately excluded from the multi-layer sibling capture below. That guard was written when `duplicateLayer` left the *pre-duplicate* selection in `selectedLayerIds`; since #808 (#804) the action resets `selectedLayerIds` to just the copy, so the guard is now belt-and-braces. Either way an option-drag moves exactly one layer, even when several were selected before it started.
...
- **A drag with an active marquee leaves the float live after release**, just as a transform-handle drag does — a follow-up drag keeps moving the same pixels, and nothing is baked until Escape, the `⌘D` key, undo/redo, a click on another layer's row in the Layers panel, a canvas press with another tool, or (since #808) `Delete`. Two consequences are easy to hit. The next canvas press with **any other tool** commits the float by re-selecting the **whole layer's** alpha, so the marquee grows to cover everything on the layer (see [Commit lifecycle](#commit-lifecycle)). And a **`⌘X`** is reverted by the next drag (under [Clipboard](#clipboard)). `Delete` in that state used to clear the entire layer; #808 made it clear just the moved marquee (see [Clipboard](#clipboard)). Five more actions go wrong in that state, because they neither commit the float nor account for it:
...
- **Dragging inside a transformed selection moves just the transformed pixels (#786, fixed in #791).** After rotating or scaling a marquee with the transform handles, a drag inside the box reuses the live GPU float — its bounds become the selection's post-transform bounding box, and the transform's original mask is carried over to rebuild the selection on release. Until #791 that drag committed the transform and re-selected the **whole layer's** alpha first, so it floated every opaque pixel on the layer and dragged unselected pixels along with the piece. The old commit-and-reselect path now runs only when the engine holds no float to reuse.
  - **The drag keeps the pending rotation and scale (#806, fixed in #808).** #791's version re-rendered the float with the plain `compositeFloat(dx, dy)`, which draws the *base* float — so a rotated piece snapped back to 0° the moment it was dragged, and `⌘D` committed it unrotated. The Move press now captures the pending transform, each move composes the drag into its translation and renders through `compositeFloatAffine`, and the release re-arms the pending transform so a later handle drag, `Enter`, or `⌘D` commits the dragged-and-rotated result. During the drag the marquee's bounds just follow the pointer (the original box offset by the drag); the mask itself is materialized at the new offset on release.
...
- **Stops**: 2 - 16 color stops with position (0-1), edited in the **Gradient modal** (opened by clicking either the gradient swatch or the "Advanced…" button in the options bar — both go to the same place). Click an empty spot on the handle row to insert a stop there, drag a handle to reposition it, and select a handle to drive a full ColorPicker (SV square + hue strip + alpha bar, plus the Hex and R / G / B text inputs added in #808 — there are no numeric HSV fields; see [Color Picker](#color-picker-shared-component)) with a percent position readout. Delete is gated at the 2-stop minimum; the 16-stop cap matches the GPU uniform limit. Stops are re-sorted by position on every edit. `Escape` or an overlay click closes the modal.
...
- **A filter grows the layer to at least document size.** `ensure_layer_full_size` runs first, because the scratch FBOs are document-sized and a smaller layer texture would make the filter sample the scratch's unwritten region. The margin it adds is transparent, and for an edit-in-place filter (blur, adjust, sharpen) nothing looks different.
...
- **Duplicate Layer** (`⌘J`, menu-only accelerator): clones the active layer and its GPU texture, inserting the copy directly above the original in the same parent group. The copy is **offset +10 px right and +10 px down — it is not cloned in place.** That offset is clamped so the copy's far edge never crosses the canvas edge the original was inside: the shift shrinks to whatever room remains, and drops to **0** for a layer already as wide (or as tall) as the canvas. Duplicating a **group** copies the group and every descendant recursively, remapping the child references, and applies one offset — computed from the group row itself — to the whole subtree.
  - **The copy becomes the only selected layer (#804, fixed in #808).** Both the single-layer and the group path now set `selectedLayerIds` to just the new copy. Before, the pre-duplicate selection survived, so a Move drag straight after `⌘J` treated the original as a multi-selected sibling and dragged it along with the copy.
  - **A duplicated group lands as one contiguous block directly above the source group (#805, fixed in #808).** The copies are laid into `layerOrder` in the same relative order as their sources, immediately after the source group's own entry. Previously each copy was spliced in right after *its own* source, interleaving copies with originals — the Layers panel mis-nested the copy's children under the source group and the compositor stacked them interleaved.
...
- **Group Layers** (`⌘G`, menu-only accelerator): wraps the currently-selected layers in a new group
  ...
  - **A selected group brings its whole subtree along (#807, fixed in #808).** When the selection includes a group, the `layerOrder` block is built from every selected layer **plus all of each selected group's descendants**, still in their original relative order. Before, the new group's `children[]` were right but the nested layers stayed at their old z-positions in `layerOrder`, so the panel indented them under the wrong sibling and the compositor drew them below layers they should have covered.
...
- **Rasterize Layer**: bakes a **text** layer's current visual into pixels in place, reading the engine's current x/y/w/h so the result lands at the visible position even after GPU texture expansion from upstream paint ops. The button appears in the Layers panel toolbar only while a text layer is active — there is currently no rasterize entry point for shape or group layers.
  - **Text is also rasterized implicitly by `clearPersistentTransform` (#798, added in #808).** Rotating or scaling a text layer with the Move-tool handles bakes the transformed pixels into its texture, but the layer used to stay `type: 'text'`; the next Text-panel edit re-rendered the string from scratch at the zeroed `x`/`y`, so the transform vanished and the text jumped to the canvas corner. On commit, the transform-clearing routine now crops the texture to its content and swaps the layer for a plain **raster** layer carrying the same name, visibility, lock, opacity, blend mode, clipping, effects, mask, and color tag. No history row of its own is pushed.
  - **Known defect — it rasterizes live text that was never transformed (#812).** The conversion runs whenever `clearPersistentTransform` finds a **text layer active**, with no check that a transform was pending — and that routine runs on **every Layers-panel row click** (the handler clears the transform before switching layers), on **Escape**, and on the **`⌘D` key**. So with a text layer active, clicking any layer row (including that layer's own **Layer effects** button, which selects the row first), pressing Escape, or pressing `⌘D` silently turns it into pixels: nothing changes on screen, but font and size edits stop applying and a Text-tool click on the words starts a new text layer. No history row is pushed, and the issue's repro found `⌘Z` could not bring the text back. The Select → Deselect *menu item* only calls `clearSelection` and does not trigger it. PR #838 gates the conversion on a transform actually being pending.
...
- **Cmd/Ctrl+click a layer thumbnail**: loads that layer's alpha as a marquee selection — each pixel's alpha becomes the selection mask value (anything below 1 is dropped), so a soft edge yields a feathered selection rather than a hard one. Any live GPU float is committed first and the layer's JS pixel cache is cleared, so the selection reflects the finished pixels. It also **seeds a transform state** from the resulting bounds and schedules a prefloat, meaning the selection comes up ready to scale/rotate — **except on a text layer**, where the prefloat is skipped (#785, fixed in #791): the prefloat runs the engine's `ensure_layer_full_size`, which re-origins the layer to `(0, 0)` and breaks the anchor the text re-render relies on, so the next Text-tool click on empty canvas near the top-left was routed into that layer and snapped the text to the canvas corner. It is the same invariant #767 fixed for the Brush pre-warm; the marching-ants selection itself is unchanged.
  - **The prefloat no longer writes `(0, 0)` into the store (#802, fixed in #808).** On a raster layer the prefloat's `ensure_layer_full_size` re-origins the texture to `(0, 0)` at document size, and `executePrefloat` used to copy just that new origin into `layer.x` / `layer.y` while leaving `width` / `height` at the cropped size — a phantom rectangle at the canvas corner that Snap to Layers then pulled drags toward. It now leaves the store alone and relies on the engine's own guard in `update_layer`, which keeps the expanded descriptor while a float is active. Deselecting (`clearSelection`, e.g. the `⌘D` key or Escape) now also drops a still-live float and **crops a raster layer back to its content**, writing the cropped bounds into the store if they differ, so both sides agree again without needing a Move drag.
  - **Known defect — ⌘-click a layer that is not the active one, and the next undo throws it off the canvas (#810).** The prefloat floats the layer whose thumbnail was clicked, which does **not** have to be the active layer, but the deselect crop above only touches the **active** layer. So after ⌘-clicking another layer's thumbnail and deselecting, that layer's engine descriptor stays at `0, 0, doc-size` while the store keeps its cropped `x, y`, and nothing reconciles them. The next undo or redo runs `resetTrackedState`, `syncLayers` pushes the store's cropped offset onto the full-size texture, and the content lands at twice its offset — usually off-canvas; redo does not bring it back. The issue's logged repro (600 × 400 document, a circle at 360..480 × 90..210 on Layer 1, Layer 2 active): after ⌘-click + `⌘D` store `360, 90, 120 × 120` vs engine `0, 0, 600 × 400`; after one brush stroke and `⌘Z`, engine `360, 90, 600 × 400` and the circle gone. The old `(0, 0)` store write used to hide this gap.
...
- **Collapsed groups no longer throw the drop off (#797, fixed in #808).** The panel list omits the children of a collapsed group, so a display row number is not an index into the document's layer array. The old handler converted it with `layers.length − 1 − row`, which above a collapsed group reached into one of that group's hidden children and **moved it instead of the row being dragged**. The drop is now resolved through the dragged layer's id and the id of the row just above the gap (`resolveDisplayDropIndices`).
- **Known defect — after a Paste, a reorder can move the wrong layer (#824).** That resolver still indexes `document.layers`, while `moveLayer` reorders by `document.layerOrder`; the two arrays fall out of step after a Paste, and from then on a drop can land one slot off, do nothing, or move a different layer than the one dragged. Creating layers in place (rather than pasting) avoids it.
- **Known defect — the gap below an expanded group's last child drops at the *top* of the group (#814).** Dragging a root layer up into the gap between a group's last child and the next root row draws a full-width line in that gap, but the layer is reparented into the group as its **topmost** child — above the child it was meant to sit under, so stacking changes the opposite way. Dropping on the top part of the group's own row correctly places the layer above the whole group.

Reparenting is inferred from the gap's neighbour rather than from indentation: the item **below** the gap decides which group the drop lands in, which is what stops a layer from being sucked into a group when it is dropped at that group's lower boundary. If that neighbour resolves to a different parent than the dragged layer's — and the move is legal — the layer is reparented; otherwise the drop is a plain reorder within the current parent. **A drop that escapes outward past a group's boundary is always a flat reorder (#788, fixed in #791).** When the neighbour's parent is an *ancestor* of the dragged layer's current parent — dropping a child below its group's last child, for instance — the direct reparent is skipped and the flat reorder places the layer in the gap it was dropped into. Before the fix that case went through `moveLayerToGroup`, which appends, so the child was ejected to the very top of the outer stack whatever gap was shown.
...
- **`Delete` while a float is live clears the marquee, not the whole layer (#801, fixed in #808).** Until #808 the Delete handler answered a live GPU float by calling `selectLayerAlpha` on the active layer, which **replaced the selection with that whole layer's alpha** before clearing — so *select, drag the piece somewhere, press Delete* wiped the entire layer (verified live at the time: an opaque 400 × 300 layer, a 100 × 80 marquee dragged 60 px, then `Delete` → 0 of 120 000 pixels left), and with a selection loaded from *another* layer's thumbnail it cleared the whole active layer instead of that shape. The handler now cancels any pending prefloat, drops the float (its pixels are already composited into the layer), re-uploads the current mask, and clears with the marquee as it stands. Re-verified live on main after #808: the same marquee dragged 60 px, then `Delete` → exactly the 8 000 pixels under the moved marquee cleared (plus the 4 800-pixel strip the drag had vacated), and the selection stayed 100 × 80 at its new position.
  - **Known defect — after a transform-handle drag, `Delete` clears the *untransformed* rectangle.** A handle scale or rotate never rewrites the selection mask (it is rebuilt only at commit — see [Commit lifecycle](#commit-lifecycle)), and Delete no longer commits first, so the clear uses the pre-transform marquee. Verified live: a 100 × 80 marquee on an opaque layer rotated about 37° with the rotate handle, then `Delete` → exactly the original 100 × 80 box (8 000 pixels) cleared, the four corners of the rotated piece that stick out past it left standing, and the transform box still drawn on canvas although the engine float was gone. Commit the transform first (`Enter`) to delete what is actually on screen.
...
and swap in the new one; `flip_texture` is the exception, rendering into a temporary
texture of the layer's own size and blitting that back into the *same* layer
texture, which is why Flip alone cannot change a layer's dimensions. Each then drops the JS pixel cache so the GPU is
...
- **The mirror is about the layer's own bounding box, not the document.**
  ...
  document.
- **A layer smaller than the document no longer fills with a squashed copy of
  the composite (#795, fixed in #808).** `flip_texture` used to render the
  flipped `w × h` image into the top-left of the document-sized `scratch_a`
  texture and then blit the **whole** scratch — stale composite content
  included — back into the `w × h` layer texture, shrinking it to fit. A
  freshly pasted 120 × 120 layer flipped straight after the paste therefore
  showed a miniature of the entire document. The flip now goes through a
  temporary texture acquired at the layer's own `w × h` and released
  afterwards.
...
- **`Backspace` / `Delete`** (canvas focused) — when a marquee selection is active, clears the selected pixels on the active layer (GPU clear, undoable as "Clear Selection") — hard-edged across a feather, and, after a transform-handle drag that has not been committed, the **pre-transform** rectangle rather than the transformed piece (both under [Clipboard](#clipboard)); when no selection is active, removes the active layer from the document — **including the document's last one** (see [Layers Panel Toolbar](#layers-panel-toolbar)). Suppressed while a text layer is being edited or any `<input>` / `<textarea>` has focus — but not while a dropdown or button does (see the focus guard below).
...
**The focus guard is the element's tag, nothing else.** Every key above — the tool letters, `X` / `D` / `Q`, the brackets, arrow nudges, `Backspace` / `Delete`, `Escape`, `Enter`, `Space`, and every `⌘` combo including undo — runs through one capture-phase `keydown` listener on `window`, and its only opt-out (outside text-layer editing) is `e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement`. There is no check for an open dialog, for the Layers panel, or for any other kind of focused control, and the test is by tag rather than by input type, which cuts both ways:

- **Any `<input>` swallows everything — sliders included.** The app's sliders are `<input type="range">`, and Chromium focuses a range input when it is clicked, so after adjusting a slider every global key is ignored until focus moves elsewhere (a press on the canvas moves it). Verified live: after one click on the Brush **Opacity** slider, `E` left the tool on Brush and `⌘Z` left the undo stack untouched; with focus back on the page, the same `E` selected the Eraser and `⌘Z` undid.
...
### Help Menu
- **Tutorials** (#825): opens the static tutorials site at `/tutorials/` in a **new tab** (`noopener`), so an open document is not lost by navigating away. See [Tutorials](#tutorials).
...
- Panels re-added from the toolbar land in the **right dock**, inserted so the vertical order stays canonical (Navigator, Info, Color, Swatches, Text, Channels, History, Paths, then Layers at the bottom, mirroring the pre-dock sidebar).

### Default Layout
- A **Color/Info** tab group above a **Layers/Channels** tab group in the right dock (50 / 50 split), with **Color** and **Layers** as the active tabs; the other five panels (Navigator, Swatches, History, Paths, Text) start closed.
...
- **Hex and R / G / B text inputs (#799, added in #808)**: a row under the alpha bar with a `Hex` text field (up to 7 characters, shown uppercase with a leading `#`) and three `R` / `G` / `B` number inputs (0 – 255). Hex applies **live** as soon as the field holds a parseable 6-digit value (with or without the `#`), and otherwise commits on **Enter** or blur — which also accepts the 3-digit shorthand, while unparseable text silently reverts. Hex never touches alpha: the color's current alpha is kept. Each R / G / B input applies on every keystroke, clamped to 0 – 255, and a field emptied with Backspace sets that channel to **0** immediately rather than waiting for a new value. The row is omitted from the grayscale value-ramp variant (and from the unreachable `compact` variant), so it appears on every picker surface in an RGB, CMYK, or Lab document.
  - **The Color panel now shows two of each.** The panel already had its own `#` hex field and R / G / B / A sliders (see [Hex field and sliders](#hex-field-and-sliders)) and still renders them below the picker, so an RGB document's Color panel carries **two hex fields and two sets of R / G / B controls** editing the same swatch. Verified live on a fresh RGB document: the panel's inputs are `Hex color` (`#000000`), `Red` / `Green` / `Blue` number inputs, then `Hex color value` (`000000`) and the R / G / B / A slider-plus-text pairs. The two hex fields also disagree on form — the picker's takes 7 characters including `#`, the panel's 6 without. In CMYK and Lab documents the picker's row is the only place R / G / B can be typed. Both routes go through the panel's `convertColorToDocMode` wrapper, so neither bypasses mode clamping.
...
A saved, reusable palette, toggled from the **Swatches** (swatch-book) icon in the [Panel Toolbar](#panel-toolbar). It starts closed and docks under Color. Unlike [Recent colors](#recent-colors), which records what you painted with and evicts the oldest entry, swatches stay until you remove them, and they **persist across reloads** (`localStorage` key `lopsy-swatches`). Swatches belong to you, not to the document, so they are not stored in `.lopsy` files.

- **Grid**: 20 px cells that wrap to the panel width and scroll after 240 px, capped at **256** swatches (the same ceiling as an Indexed palette). The swatch matching the current foreground color (exact RGBA) gets the accent ring and `aria-pressed`. The tooltip shows the name (if any), the hex, and the modifier hints.
- **Click** sets the foreground color. **⌘/Ctrl-click** sets the background color. **⌥/Alt-click** deletes the swatch. Both color routes go through `convertColorToDocMode`, like the Color panel, so picking a chromatic swatch in a Grayscale document gives its luminance, and in an Indexed document gives the nearest palette entry.
- **Deleting is pointer-only.** A swatch has no Delete / Backspace key handler: the app-wide shortcut handler listens on `window` in the capture phase and treats Delete on a focused button as "delete the active layer", so a per-swatch key would also delete a layer.
- **Add Foreground Color (+)** adds the current foreground color, alpha included.
- **Every add path removes duplicates on exact RGBA.** Adding a color that is already present does nothing, and the same RGB at another opacity counts as a different swatch (the same rule as Recent colors).
- **Extract Colors from Image** (pipette + count dropdown of 4 / 6 / 8 / 12 / 16, default 8): flattens the visible document with the export compositor and appends up to *N* dominant colors, sorted neutrals first (dark to light), then around the hue wheel in 30° buckets, each bucket ordered dark to light. A toast reports how many were added, or that all of them were already present. Transparent pixels are ignored, so a sticker on a transparent canvas yields only its painted colors, and the extracted entries are always opaque. Pending brush / dodge strokes are finalized first so the last stroke counts.
  - **Engine**: `extractCompositePalette` → `lopsy_core::quantize::dominant_colors`: the same median-cut seed that Image → Mode → Indexed uses, **refined with up to 16 k-means (Lloyd) iterations** over a sample of at most 64 K pixels (fixed stride, no randomness). Median cut on its own splits boxes at the pixel-count median, so a large flat background takes most of the splits and small distinct colors get averaged together. In the e2e test (white canvas plus orange, jade, and violet squares), median cut alone returned `#df5142`, `#319ca7`, and `#dcd6f3`, none of which appear in the image. A cluster left empty is moved onto the sample that fits its center worst, one per pass, which is how an averaged-away color gets its own entry. With refinement, a flat-color image comes back exactly. Indexed conversion still uses plain median cut and is unchanged.
- **Import Palette (.gpl)**: reads a GIMP palette (the de-facto interchange format, also written by Krita, Inkscape, Aseprite, and Lospec). It needs the `GIMP Palette` header; `Name:` is read, and `Columns:`, `#` comments, and unrecognized lines are skipped. Channels above 255 are clamped. Entry names are kept (GIMP's `Untitled` becomes unnamed). Imported colors are **appended** with duplicates removed, and a toast reports the count and palette name. A file without the header or without colors is rejected with an error toast and leaves the palette untouched.
- **Export Palette (.gpl)**: downloads `lopsy-swatches.gpl` (`Name: Lopsy Swatches`, `Columns: 8`). Unnamed swatches are written with their hex as the name. **Alpha is dropped**, because GPL has no alpha channel, so a translucent swatch re-imports as opaque.
- **Reset to Default Swatches** restores the 24 built-in named swatches (5 neutrals, 9 saturated hues, 5 pastels, 5 earth tones) without asking first, and it **cannot be undone**: swatch edits are not part of document history.
...
The hex field stays available in every mode. In RGB, CMYK, and Lab the picker also adds its own Hex + R / G / B text row (#808); Grayscale's value ramp and Indexed's palette grid do not.
...
- **Exactly three things, all in `localStorage`.** The app uses no IndexedDB, no cookies, no OPFS, and no Cache API anywhere in `src/`.
...
- **Header**: the `LOPSY` wordmark in Jersey 10, matching the editor's own logo (#830), plus *Tutorials* and *Open Lopsy* links.
- **Published as of this audit (4)**: *Make a Neon Glow Text Effect* (Beginner, 5 min), *Make a Vaporwave Sunset Billboard* (#829, Intermediate, 45 min), *Design a Propaganda Poster Style Party Invitation* (#832, Intermediate, 60 min), and *Design a Swiss Style Exhibition Poster* (#834, Intermediate, 50 min).
