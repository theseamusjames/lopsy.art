...
=======
- **Displacement Map**: uses another layer's pixel values to displace the active layer. Source Layer picker selects the displacement map; Horizontal Scale -200 to +200 and Vertical Scale -200 to +200 control displacement magnitude in pixels. Channel Mode: Red/Green (red channel drives X, green drives Y) or Luminance (brightness drives both axes). Edge Behavior: Transparent (out-of-bounds pixels become transparent) or Tile (wraps around). Supports live preview with debounced updates. GPU shader computes `srcUv = v_uv + offset * texel` where offset is derived from the displacement texture's channel values.
>>>>>>> 65b4b10 (feat(filter): add displacement map filter)
