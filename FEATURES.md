...
**Known defect (#767) — picking up the Brush while a text layer is selected makes the next property change teleport the text to the canvas corner.** Re-rendering a committed text layer rests on one invariant, spelled out in `rerenderCommittedTextLayerAnchored`'s own doc comment: *"A committed layer stores its texture top-left in `x`/`y`, which equals `anchor + oldRenderOffset`."* It recovers the anchor as `layer.x − renderOffset` and places the new texture there. **The brush pre-warm quietly breaks that invariant.** `shouldPrewarmStroke` refuses locked layers and refuses groups — but it does **not** refuse text layers, so merely *selecting* Brush, Pencil, or Eraser with a text layer active (no click, no stroke) fires `prewarmStroke`, whose `ensure_layer_full_size` re-origins the layer to `(0, 0)` at document size, and `syncLayerAfterFullSize` writes that straight into the store. The type still looks right, because the glyphs are baked into the doc-sized texture at their absolute position — but `x`/`y` now say `(0, 0)`, so the recovered anchor is `0 − renderOffset` and the next options-bar or Text-panel change re-renders the type at the document origin.

Verified live on a 400 × 300 document, one text layer reading `WORLD`, A/B from the same starting state:

| Route | `layer.x/y` + texture after commit | Regular → Bold | Result |
| --- | --- | --- | --- |
| Commit by switching to **Move** | `(97, 233)`, 97 × 27 | applied | text **stays at (97, 233)** ✓ |
| Then select **Brush** (nothing else) | `(0, 0)`, **400 × 300** | applied | text **jumps to the canvas corner** ✗ |

Selecting the Brush is enough on its own — the second row's only extra step was clicking the Brush button. Any other full-size expansion of a text layer does the same thing (a paint stroke, a filter), and every subsequent property change re-anchors from the corner. It is **not** new in #759, whose anchor cache only skips the recovery *during a slider drag*: the same A/B reproduces on the parent commit `e18a9d48`. Undo restores the position along with the property. The practical workaround is to finish a text layer's styling before reaching for a paint tool — or to nudge it back with the Move tool afterwards, which rewrites `x`/`y` to values the anchor recovery can use again.
...
- **Known defect (#766) — the "generators" cannot generate onto transparency, so on an empty layer they do nothing at all.** Clouds, Smoke, and Fibers each end their shader with `fragColor = vec4(vec3(pattern), orig.a)` — the pattern is written into **RGB only** and the source pixel's alpha is passed straight through. Add Noise (`c.a`) and Voronoi (`cellColor.a`) do the same. So the expansion above buys them nothing: the new margin is alpha-0, the shader dutifully fills it with cloud values, and every one of those pixels stays invisible. The visible result is clipped to whatever alpha the layer already had. Verified live on a 400 × 300 document: **Clouds on the empty `Layer 1` of a brand-new document** expanded the texture to full size and wrote a cloud pattern into 109,626 pixels' RGB, but left `max(alpha) = 0` across the whole texture — the canvas never changed. Running it on a layer holding one small black ellipse expanded the texture to 403 × 302 and wrote cloud RGB into **117,019** pixels, while the opaque count stayed at the ellipse's own (28,756 before → 29,188 after): the clouds render **inside the ellipse and nowhere else**. The control — the same Clouds on the opaque white Background — filled the canvas as expected. Treat these five as *alpha-preserving recolorings*, not generators: to get a full-canvas cloud, fill the layer with something opaque first. (This corrects an earlier claim in this document that the expansion makes a generator "cover the entire canvas"; it is the expansion that is real, not the coverage.)
...
- **Known defect (#771) — applying a filter to a layer that is not already document-sized at the origin permanently relocates it by its own `(x, y)`.** The expansion re-origins the *engine's* copy of the layer descriptor — `ensure_layer_covers` writes `layer.x = min_x`, `layer.y = min_y` and the new dimensions — and nothing tells the Zustand store, which keeps the pre-filter position and size. `sync-layer-after-full-size.ts` exists to close exactly that gap, and its own doc comment states the invariant: the JS side "**must** be updated to match — otherwise the next `syncLayers` push would clobber the engine's expanded dimensions with the stale JS values". `layer_manager::update_layer` even carries a guard that preserves the engine's bounds against that push — but only when the layer is the *float* layer. The paint tools, the gradient, Smudge, the bucket, and the brush pre-warm all call the reconciler, and the [Shape tool](#shape-tool) keeps its own copy of it; **the menu filters call neither — and neither does anything else driven from the menu bar.** Not one module under `src/app/MenuBar/` references either reconciler, so Pattern Fill, Color LUT, Liquify, Mesh Warp and the Tilt-Shift session all commit through the same unreconciled path. The desync then stands until the next change to that layer's descriptor — any one at all: visibility, opacity, blend mode, rename, an effect toggle — pushes `updateLayer` with the stale bounds, and the composited layer jumps by `(x, y)`. The jump is **baked in** by the next crop-on-leave / expand-on-return cycle (see [Layer Texture Lifecycle](#layer-texture-lifecycle-crop-on-switch-expand-on-return)), which re-reads the texture and agrees with it, so afterwards nothing is left to signal that the pixels moved; only the filter's own undo entry gets them back, and there is no toast at any point. Verified live on a 400 × 300 document: a stroke dragged 50 px right left the layer cropped to `x 125, y 73, 110 × 51` with store and engine agreeing; `Filter → Invert` grew the texture to `400 × 300` while the store still read `110 × 51 at (125, 73)`; one visibility toggle in the Layers panel moved the stroke's opaque bounding box from `(125, 73) – (234, 123)` to `(250, 146) – (359, 196)` — exactly `+ (125, 73)`. A layer that is *already* at `(0, 0)` at document size is unaffected, and that is what the expand-on-return cycle hands you for most freshly-activated layers, which is why the defect survives ordinary use: it needs a layer whose texture has been cropped to its content — most reliably, one that has just been moved.
...
**Painting with a group active paints into a texture nothing ever draws.** `handleToolDown` gates on the active layer's *lock* but never on its *type*, so with a group row selected the Brush, Pencil, and Eraser run their ordinary GPU stroke: `pushHistory` records a row and `beginStroke` gives the group a document-sized texture of its own, which the dabs land in. The compositor, however, never samples a group's texture — its group branch composites the children (through the group buffer when the group has adjustments) and then `continue`s past the per-layer blend, on the stated assumption that "groups have no texture". The stroke is therefore invisible, no toast says why, and each attempt still costs an undo row and empties the redo stack. The same applies to the root group once a document's last real layer has been deleted (see [Layers Panel Toolbar](#layers-panel-toolbar)). Verified live: with a new empty group active, one Brush click pushed `Brush`, left 76 opaque pixels in the *group's* texture, and left the Background layer beneath byte-for-byte unchanged.

**And it is not only the three paint tools — nothing in the app checks a layer's type before writing pixels to it (#768).** A census of every pixel-writing entry point found **no `type` guard anywhere**: not in `handleToolDown`, not in any of the twelve tool `down` handlers (`paint-handlers.ts` does not reference `.type` at all, and the dodge / sponge / smudge / stamp / healing / spray modules only read `activeLayer.x/y`), and not in the menu-driven writers (`fillSelection`, `filter-actions.ts`, `definePattern`). The three `type` comparisons that do appear in tool code — in `shape-interaction.ts`, `move.ts`, and `perspective-crop-interaction.ts` — are bounds arithmetic, not gates. So every one of these lands on a group exactly as the Brush does. Verified live on a 400 × 300 document with an empty group selected, each run on its own from a clean undo stack:

| Gesture with a group active | Group texture after | History row | Canvas |
| --- | --- | --- | --- |
| Bucket Fill click | 1 × 1 → **400 × 300, 120,000 px opaque black** | `Bucket Fill` | unchanged |
| Linear Gradient drag | 1 × 1 → **400 × 300, 120,000 px**, mean grey 121 | `Linear Gradient` | unchanged |
| Shape (ellipse) drag | 1 × 1 → **507 × 393, 87,513 px opaque** | `Shape` | unchanged |

In every case the Background beneath stayed white and the screen never changed. Undo reverses the invisible write normally, so the only lasting cost is the wasted history row and the cleared redo stack.

**A text layer is the opposite case: the write lands and shows, and the next text edit throws it away.** Text layers do own a texture the compositor samples, so a brush stroke, fill, or gradient aimed at one is immediately visible — but it lives in the same texture the type rasterizer owns. Verified live: a brush stroke across a committed `HELLO` layer took its texture from 83 × 27 / 566 opaque pixels to 400 × 300 / 1,604, and the stroke drew on screen; changing that layer's weight from Regular to Bold re-rasterized the texture back to 83 × 27 / 566 and the stroke was **gone**, with only the ordinary `Text` history row to show for it. The layer stays `type: 'text'` throughout — it is never promoted to raster — so there is no state in which the painted pixels are safe. Rasterize the text layer first if you mean to paint on it. Selecting a paint tool while a text layer is active has a second, separate consequence — see [Text Tool](#text-tool).
...
- **Fill** (Edit menu, `⇧F5` — menu-only accelerator, not wired to a global key): fills the current selection (or the entire layer if no selection) on the active layer with the **foreground color**, immediately. There is no dialog and no color/pattern choice — to fill with anything else, set the foreground color first or use Fill with Pattern. The fill runs on the GPU and honors the active selection mask.
  - **Known defect (#765) — it does nothing on a layer that has never been painted.** `fillSelection` calls `fillWithColor` without first running `ensure_layer_full_size`, so on a layer whose GPU texture is still the lazy **1 × 1** placeholder it blits the document-sized scratch FBO into that one pixel and stops. The layer is not expanded, the canvas does not change, and the single pixel it does write is **not even the foreground color** — it is whatever the compositor scratch happened to hold. A `Fill` history row is pushed regardless, so the redo stack is cleared for nothing. That state is the default for every layer Lopsy hands you: the `Layer 1` of a brand-new document is empty, so *New Document → Edit → Fill* is a no-op. Verified live on a 400 × 300 document with a black foreground: Edit → Fill on the untouched `Layer 1` left it **1 × 1, one opaque pixel, RGB (255, 255, 255)**; the [Bucket Fill tool](#fill-paint-bucket) clicked on that same layer filled it **400 × 300, all 120,000 pixels black**. The bucket is the one that gets it right — its `isLayerEffectivelyEmpty` fast path routes to `bucketFillSolid`, which does call `ensure_layer_full_size` (#667). The control for the menu path was Edit → Fill on the Background, whose texture is already 400 × 300: all 120,000 pixels went black correctly. **Paint one dab (or bucket-fill) first and the menu command works from then on** — as long as the layer is still document-sized. **The 1 × 1 placeholder is only the extreme case**: the missing expansion hurts *any* layer whose texture is smaller than the document, for the same reason the filters need it (the scratch FBO is document-sized, so a smaller destination samples the scratch's unwritten region). Verified live on the same document: a stroke dragged 50 px right, leaving the layer cropped to `110 × 51 at (125, 73)`, then Edit → Fill — the texture stayed `110 × 51` and read back fully opaque black, but the canvas came back showing a **squashed ~30 px-wide block** of color plus a shrunken ghost of the stroke instead of the filled `110 × 51` rectangle. Unlike the filter case (#771) the bounds never desync here, because `fill_with_color` never expands at all; what is wrong is the pixels.
...
- **Known defect — the stack and the canvas disagree about groups in both directions.** A Brush, Pencil, or Eraser stroke made while a group is active records a real row for a stroke that can never be seen (see [Layer Types](#layer-types)), while **New Group** changes the document and records no row at all, so the next undo takes the group away together with the edit before it (see [Layers Panel Toolbar](#layers-panel-toolbar)).
