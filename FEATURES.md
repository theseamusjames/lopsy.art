...
- **CRT / Scanline**: scanline intensity 0 - 100% (darkness of horizontal scan bands), scanline spacing 1 - 8 px (distance between scanlines), curvature 0 - 100% (barrel distortion simulating a curved CRT screen), phosphor glow 0 - 100% (RGB sub-pixel separation and horizontal bloom mimicking phosphor bleed), vignette 0 - 100% (edge/corner darkening typical of CRT displays). GPU-accelerated single-pass shader that simulates a retro CRT monitor — creates vintage gaming, VHS, and old-TV aesthetics.
