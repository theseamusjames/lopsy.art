...
- **Undo** (`⌘Z`) / **Redo** (`⇧⌘Z`) from the Edit menu or keyboard
  - **The keyboard shortcuts are deferred to the next animation frame** (#761, shipped in #764). `⌘Z` / `⇧⌘Z` no longer call `undo()` / `redo()` from the `keydown`; they go through `scheduleUndoRedo`, which counts presses — the first one and every auto-repeat — and applies them once per `requestAnimationFrame` as a single batched `undoBy(N)` / `redoBy(N)`. Holding `⌘Z` on a masked 4K document therefore pays one mask re-upload per frame instead of one per repeat. Flipping direction mid-batch applies the pending batch first so the two do not cancel. The Edit-menu items still call `undo()` / `redo()` directly and apply synchronously. The cost is that a keyboard undo is not visible until the next frame runs: the history stack is unchanged immediately after the keypress, and in a background tab, where the browser pauses animation frames, the undo waits until the tab is shown again.
