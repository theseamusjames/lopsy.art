...
- **Hardness**: 0 - 100%. **Hardness 100 reaches full alpha at every size (#849, fixed in #865).** `circleStamp` floors the radius it tests coverage against at ~0.81 px (`kNearestPixelOffset` 0.71 + 0.1) and narrows the anti-aliasing rim below 1 px only as far as that floor requires, so the pixel centre nearest any dab — up to √2⁄2 px away — always lands in the flat, fully covered core. Before, the fixed 1 px rim (`radius − 1 … radius`) swallowed a small dab's whole footprint: at radius ≤ ~0.7 px (Size 1) every pixel was discarded and nothing painted, and up to ~1.7 px no pixel reached full alpha even at Hardness 100 (Size 2 peaked at 53/255). Above radius ~1.7 px (about Size 3.5) the stamp is unchanged. Spray's dots go through the same shader.
...
- **Speed Size**: stroke velocity modulates brush size. A `Faster is` toggle picks the direction (`Thinner`: `scale = 1 − (pct/100) × speed`, range 0 – 100%, so a fast stroke is driven to the 1 px floor; `Wider`: `scale = 1 + (pct/100) × speed`, range 0 – 300% — the percentage is what gets *added* to the base, so the top of that range is **4× the base size**, not 3×; switching back to `Thinner` clamps a value above 100 down to 100). Raw velocity is normalized against a 5 px/ms ceiling and clamped to 1, then averaged over a sliding window — and **that window length is what the `Sensitivity` toggle sets**: Low = 6 samples, Med = 3, High = 2, so Low is the most damped and High the twitchiest. The resulting size *scale* is then eased toward its target by a further 0.25 blend per dab, which is what keeps the width from stepping visibly.
  - **The scale resets to 1× at every pointer-down, and the easing is counted in dabs, not in time or distance.** A fresh stroke state is built per stroke and sets neither `speedSizeCurrent` nor `speedHistory`, so the walk always restarts from `1` (base size) with an empty speed window. Each dab then closes a quarter of the remaining gap, so the gap falls by `0.75ⁿ` and it takes **11 dabs to cover 95 % of the distance** to the target scale. At `Wider` 300 % and full speed the first dab is only **1.75×**, the third **2.73×**, and 4× is never quite reached. The practical consequence is that the ramp is a property of stroke *length in dabs*, so **Spacing silently sets how far you must travel to feel it**: at the built-in presets' 1 % spacing the 11-dab ramp is about 11 % of one brush width, but at Spacing 200 % the same 11 dabs span 22 brush widths. Short flicks therefore show almost no speed response however high the slider is set. (Speed Size is a Brush-only dynamic — no other dab engine reads it; see [Dab Engines](#dab-engines-shared-across-paint-tools).)
...
- **At startup the highlighted preset is not the live brush.** The store ships `activePresetId: 'builtin-hard-round'` in its initial state, but `setActivePreset` — the only thing that copies a preset's parameters into `settings.brush` — is called solely from user actions, never at boot. A fresh session therefore paints with `DEFAULT_BRUSH_SETTINGS` (**hardness 80, spacing 0**) while the Presets grid shows Hard Round (**hardness 100, spacing 1**) as the selected preset. Clicking the already-highlighted Hard Round is not the no-op it looks like: it hardens the brush from 80 to 100.
...
- **The scratch texture now grows to cover the layer before the eraser runs (fixed incidentally by #883).** `scratch_texture_a` starts at document dimensions, while the eraser's viewport and its blit use the *layer* texture's dimensions — so erasing on a layer expanded past the document bounds used to sample and write beyond the scratch texture's extent. The Eraser is a registry paint tool, so the shared pointer-down path calls `beginStroke`, whose `ensure_layer_full_size` → `ensure_layer_covers` now also calls `ensure_scratch_covers` (added by #883 for #862), growing both scratch textures (and re-attaching their FBOs) to at least the layer's texture size before the first dab. The scratch only grows; it returns to document size only when `set_document_size` reallocates it. `end_stroke` keeps its own temporary-texture guard for brush strokes. The mask dab batches never reach `beginStroke` and are unaffected.
...
- **Smudge guards its own layer size because it is not a registry paint tool.** It calls `ensure_layer_full_size` before dabbing, with a comment naming the failure it prevents: the dispatch writes to a scratch FBO with the viewport set to the *layer* texture size, so a layer smaller than the scratch blits back garbage as full-width streaks. The Eraser, Clone Stamp, and Healing Brush get the same call from the shared pointer-down `beginStroke` (`begin_stroke` in `brush_gpu.rs`), which since #883 also grows the scratch to the layer's size — see [Dab Engines](#dab-engines-shared-across-paint-tools).
...
- Holding the cursor still keeps emitting dots on a 166 ms timer (~6 Hz) so paint accumulates over time, mimicking an airbrush. Dragging emits a fresh cloud each time the pointer has travelled `max(1, 0.3 × Size)` px, so consecutive clouds overlap by about 70% of their width at every Size.
  - **Known defect — slow drags don't spray, and the hold timer stays at the pointer-down point (#793; fixed in #808, reverted by #839).** Two bugs in `spray-interaction.ts`. A move shorter than the spacing still advances `state.lastPoint` to the pointer *without* emitting, throwing the travelled distance away — so a natural-speed drag made of many small pointer events lays down nothing after the pointer-down cloud; only single moves longer than `max(1, 0.3 × Size)` px spray. And the 166 ms timer closes over the state object `handleSprayDown` returned, while the dispatcher keeps a spread copy of it (`withPaintGesture` / `withToolGesture`) whose `lastPoint` is what pointer-moves update — so the "airbrush" keeps spraying at the **pointer-down point** for as long as the button is held, wherever the cursor goes.
...
- **Spray dabs accumulate in the stroke texture like the Brush's (#787, fixed in #791).** Until #791 `handleSprayDown` called `pushHistory('Spray')` on entry, and `pushHistory` runs `endStroke` first — which *removed* the stroke texture the shared paint-tool path had just opened, before the first dot was emitted. `apply_dab_batch` then fell back to `stroke_tex.or_else(layer_texture)`, so every dot MAX-blended premultiplied values **directly onto the layer**: spraying a darker color over lighter opaque pixels did nothing, and a saturated color on a transparent layer came out as grey or dark-mauve dots, because the per-channel max left stored RGB behind the max alpha. The redundant push is gone — history and `beginStroke` are already handled upstream for every paint tool — so dots now build up in an initially-empty stroke texture composited over the layer at `end_stroke`, and one spray stroke leaves **one** `Spray` row (the shared path used to label it `Eraser`, on top of Spray's own entry).
...
**What every non-brush engine drops.** The Brush's Spacing setting reaches only the Brush; none of the others reads it, nor the tip bitmap, the four jitters, scatter, angle, texture, flow, fade, taper, speed-size, or sub-brushes. This is the same shape of gap documented for [mask editing](#editing-a-layer-mask) and the [Eraser](#eraser), and it has the same cause: each is a second implementation that was never grown to match the first.
...
The two interpolators also place dabs differently along a segment that *is* long enough. `interpolateFlat` puts them at exact multiples of the spacing and stops at the last whole one, so up to one step of the segment's tail is left unstamped; Spray instead divides the segment into `floor(dist / spacing)` equal parts and always lands a dab on the endpoint, which stretches its real pitch to anywhere in `[spacing, 2 × spacing)`.
...
Every one of these is closed by the same 1 px `edge = 1 − smoothstep(radius − 1, radius, dist)` rim, which is the *only* thing that takes the second family to zero. The one exception is the brush's own small dabs: since #865 `circleStamp` floors its coverage radius at ~0.81 px and, below radius ~1.7 px, narrows the rim to `coverageRadius − 0.71` (clamped 0.05 – 1) — see [Brush](#brush) → Hardness.
...
**The shared scratch texture now covers an expanded layer (fixed incidentally by #883).** `scratch_texture_a` starts at document dimensions while the Eraser, Clone Stamp, and Healing Brush set `gl.viewport` and blit at the *layer* texture's dimensions, so painting on a layer expanded past the document bounds used to read and write outside the scratch texture's extent. All three are registry paint tools, so pointer-down runs `beginStroke` → `ensure_layer_full_size` → `ensure_layer_covers`, and #883 (the #862 filter fix) made `ensure_layer_covers` call `ensure_scratch_covers`, which grows `scratch_texture_a` / `_b` to at least the layer's size and never shrinks them until `set_document_size`. Smudge, Dodge / Burn, and Sponge reach the same function through their own `ensure_layer_full_size` calls, and `end_stroke` keeps its temporary-texture guard for brush strokes. Mask dabs (`mask_paint_gpu.rs`) never reach `beginStroke` and still set a mask-sized viewport on the shared scratch.

**One stroke, one undo entry — the phantom "Eraser" row is gone (#887, fixed in #895).** The shared paint-tool path at pointer-down pushes a history entry labeled `Brush`, `Pencil`, or `Spray` for those three tools and `Eraser` for the Eraser. It used to push that `Eraser` fallback for *every* other paint tool too — and Dodge / Burn, Sponge, Clone Stamp, and Healing Brush each push their own correctly-labeled entry from their own handler, and `pushHistory` does not coalesce, so every stroke with one of them left a spurious **"Eraser"** row ahead of the real one, and undoing it took two steps. Those four now carry `ownsHistoryEntry` in the tool registry, and the shared path skips its push for the derived `SELF_HISTORY_PAINT_TOOLS` set, so one stroke leaves exactly one row (`Dodge`/`Burn`, `Saturate`/`Desaturate`, `Clone Stamp`, `Healing Brush`). A Clone Stamp or Healing Brush **Alt/Cmd+click that only sets the source** now pushes nothing, because the handler returns before its own `pushHistory`, and neither does a click made before any source is set; both used to leave a lone `Eraser` row. Spray lost its own duplicate push earlier, in #791 (see [Spray](#spray)). (Smudge was never affected — it is not registered as a paint tool, which is also why it has no straight-line origin. In layer-mask edit mode and Quick Mask the shared setup is skipped entirely, and there these tools paint the layer instead of the mask; see [Editing a Layer Mask](#editing-a-layer-mask).)
...
### Shape Tool
- **Shape types**: ellipse, polygon. A square is drawn as a 4-sided polygon
  (set sides to 4); triangles use sides=3, etc. There is no way to draw a
  non-square rectangle.
  - **Known defect — Sides = 4 is a square, not a rectangle (#794; fixed in #808, reverted by #839).** Every side count goes through `sdPolygon` in `shape_fill.glsl`, which fits a *regular* polygon inside the drag box, so in Pixels output a 4-sided drag paints a **square** of side `min(w, h)` centred in the box, whatever its aspect. The shader's `sdRect` branch — the one that would honour the full drag aspect — runs only when `u_sides < 3`, which the 3 - 64 Sides field never sends. In Path output `polygonToPathAnchors` spaces the vertices around the drag's ellipse starting at the top (`2πi/n − π/2`), so the same drag gives a stretched **diamond** with vertices at top / right / bottom / left.
...
- **Stroke width**: 1 - 50 px, centred on the shape's edge — half inside, half outside.
  - **The stroke is the width the slider says (#826, fixed in #838).** `shape_fill.glsl` used to test `abs(d)` against the full width, painting `strokeWidth` px on *each* side of the edge — twice the value. It now uses `halfW = u_strokeWidth × 0.5`.
- **Polygon sides**: 3 - 64
...
- **It reconciles its own bounds — and did so before the menu writers did.** A shape drag can push the layer past the canvas edge, so `shape_gpu.rs` calls `ensure_layer_full_size` like every other GPU writer — but `shape-interaction.ts` carries its own private copy of the JS-side reconciliation (`syncLayerBoundsFromEngine`, reading the engine's descriptor back through `getLayerEngineBounds`) rather than the shared `syncLayerAfterFullSize`. It is the **third** implementation of that same reconciliation, after the shared helper and the expand-on-return path in `useCanvasRendering.ts`, and it is why shapes never suffered the filter relocation that #772 fixed (see [What every filter does to the layer](#what-every-filter-does-to-the-layer)). Verified live: a shape drawn partly off the left edge of a 400 × 300 document left store and texture agreeing at `x −29, y −4, 429 × 304`.
- **Anti-aliased shape edges are written as straight alpha (#815, fixed in #856).** `shape_fill` used to blend its coverage-weighted — premultiplied — colour with fixed-function `ONE, ONE_MINUS_SRC_ALPHA` into a straight-alpha layer texture, which the compositor then read as straight: a dark fringe round every shape. The shader now composites "over" itself against a copy of the layer's prior content (the drag preview, or a temporary copy for a one-shot shape) and writes un-premultiplied RGB.
...
- **Clicking empty canvas while a path is selected starts the next path (#789, fixed in #791).** A committed path stays selected, so the next press on empty canvas lands in the edit-mode branch; it deselects the path and then falls through to creation mode, so that same click places the first anchor of the new path. Before the fix the click was consumed as a bare deselect, and three clicks for a second path produced a two-anchor path starting at the second click.
...
- **Family names match case-insensitively, and a weight the family has no face for is instanced or snapped instead of falling to Inter (#842, #827, fixed in #856).** fontdb and cosmic-text compare family names case-sensitively, so a request for `IM Fell DW Pica SC` never matched the face named `IM FELL DW Pica SC` and rendered in Inter; the engine now resolves the requested name to fontdb's stored spelling first (`resolve_family_name`: exact, then ASCII-case-insensitive). cosmic-text also uses a family's face only at an *exact* weight, so a variable font — loaded as one default-instance face — rendered one weight and sent every other choice to Inter. `resolve_weight` now takes an exact face when there is one; otherwise, when a loaded face's `wght` axis spans the request, it bakes a **static instance** at that weight (`variable_instance.rs`: skrifa draws the gvar-varied outlines with HVAR advances, and the variation and hinting tables are dropped) and registers it as another face of the family, once per family/style/weight; otherwise it snaps to the nearest weight the family ships (heavier wins a tie). Like the style snap, this is engine-side only — the Weight dropdown keeps showing the requested value.
...
- **Vertical**: a third toggle in the same decoration group as `U` and `S`, glyphed as a stacked **A** over **B**. It stacks each glyph below the previous one instead of running them left to right — see *Vertical text* below for the full layout and editing model. Unlike Underline and Strikethrough, which are duplicated in the Text panel, **this toggle lives only in the options bar**; the panel has no vertical control.
...
- **Clicking an existing text layer hit-tests an estimated box, not the glyphs.** `hitTestTextLayer` walks the text layers from the end of `layers`, skipping hidden and locked ones, and tests the click against `x, y` plus an estimated size. Area text uses its stored `width`; point text uses `longest hard line × fontSize × 0.6` (#845, fixed in #867 — it used the character count of *all* lines, so a three-line menu column hit-tested as wide as its lines laid end to end, and a click well to its right reopened it and appended to its last line). Height is `fontSize × lineHeight × hard lines + paragraphSpacing × (hard lines − 1)`; #867 added the paragraph-spacing term. Only hard lines are counted, so an area-text layer's word-wrapped lines fall below its box. Vertical layers use their own estimate (see *Vertical text* below).
...
- **Bind to path**: a Path dropdown in the text options bar lists every stored path. Once bound, glyphs are placed one by one along the path's arc-length and rotated to match the local Bezier tangent (works on both open and closed paths). Live editing (typing) re-flows the type along the curve in real time, and editing the path's anchors invalidates the cached layout so the text follows. Selecting "None" unbinds and restores the layer's pre-bind position.
  - **Binding a committed layer is undoable (#872, fixed in #876).** The Path dropdown's handler now calls `beginTextLayerHistory()` before writing `pathId` — the same helper the style, align, and decoration controls use — so it pushes a `Text` row for a committed layer (while the layer is being edited, the commit records it). Before, the change pushed nothing, and the next `⌘Z` undid the *text commit* instead, leaving an empty `Text N` layer.
  - **Path text is redrawn once its web font arrives (#823, fixed in #856).** Canvas2D `fillText` never waits for a font — while a face is loading it draws a fallback and nothing redraws it. `syncPathTextLayers` now asks `document.fonts` to load the faces each render needed (`ensureFontFacesLoaded`, requested once per font/text pair so a face that never loads cannot loop) and drops the layer's cache key when they settle, and a `loadingdone` listener (`onFontFacesLoaded` → `pathTextLayersUsingFamilies`) re-renders any path-text layer whose family just finished loading, e.g. when the stylesheet arrived late. The picker's name-only preview subsets no longer share the family's name (see *Previews* below), so their few glyphs can't leak into that render either.

**Selecting a paint tool no longer moves a text layer (#767, fixed in #770).** Re-rendering a committed text layer rests on one invariant, spelled out in `rerenderCommittedTextLayerAnchored`'s own doc comment: *"A committed layer stores its texture top-left in `x`/`y`, which equals `anchor + oldRenderOffset`."* It recovers the anchor as `layer.x − renderOffset` and places the new texture there. The brush pre-warm used to break that invariant: merely *selecting* Brush, Pencil, or Eraser with a text layer active fired `prewarmStroke`, whose `ensure_layer_full_size` re-origined the layer to `(0, 0)` at document size, so the next options-bar or Text-panel change re-rendered the type at the canvas corner. `shouldPrewarmStroke` now refuses text layers alongside locked layers and groups, and the paint tools and menu filters that could expand a text layer the same way are refused by the layer-type guard (see [Layer Types](#layer-types)). **The guard does not cover every full-size expander**: Liquify, Mesh Warp, Tilt-Shift Blur, and Color LUT do not call it, so those can still re-origin a text layer and leave the anchor recovery pointing at the corner.

**Vertical text** (the toggle above; stored as `vertical` on the layer, default off)

- **Every hard line becomes a column, and columns advance to the right.** Each `\n` starts a new column one step further right, so a two-line string sets as two side-by-side columns with the *first* line leftmost. Traditional CJK vertical setting advances columns right-to-left; this goes the other way.
- **The layout re-purposes two existing sliders.** A column is `fontSize × lineHeight` wide and the next one starts `fontSize × lineHeight + paragraphSpacing` further right, while inside a column each row is `fontSize + letterSpacing` tall. So **Letter spacing becomes the vertical gap between glyphs and Line height becomes the column's width** — at the defaults (24 px type, 1.4 line height) a column is 33.6 px wide. Glyphs are centered horizontally within their column, so a wide glyph and a narrow one share a centre line rather than a left edge, and each baseline sits `fontSize` below its row's top.
- **Wrapping is off, so an area-text box loses its width.** Vertical mode repositions glyphs after shaping, so the engine always shapes with `Wrap::None` and never calls `set_size` — the `areaWidth` a drag-created layer carries is simply never consulted. A column runs as long as its text whatever rectangle you dragged, and the only thing that starts a new column is a literal newline.
- **Alignment is inert.** Neither the vertical layout nor the vertical render branch reads `textAlign`; the letter-spacing compensation that implements left / center / right / justify exists only on the horizontal path. The dropdown stays enabled and keeps showing your choice. Point text is already inert for its own separate reason (no alignment box); in vertical mode area text joins it.
- **Underline and strikethrough are silently suppressed.** Both decorations are drawn from the horizontal run bookkeeping, which vertical mode leaves empty, so the engine forces both off whenever `vertical` is set. The two toggles stay enabled and still light up as pressed, so nothing signals that the stroke is not being drawn.
- **The caret stays a vertical bar.** It is drawn at the **column's left edge**, one row (`fontSize`) tall, beside the row the cursor sits on — not as a horizontal bar above the next glyph the way a CJK editor draws it. Past the last glyph it drops one row-advance below it.
- **Selection highlights are column-width bands** — one rect per column, spanning the full column width and running from the first selected row's top to the last selected row's bottom. A completely empty column caught inside the range still gets a single one-row marker so it stays visible.
- **Clicking picks the column by x, then the row by y**: the column is `floor(x / column advance)` clamped to the columns that exist, and then the first glyph whose vertical midpoint falls below the click wins (a click below every glyph lands at the column's end). Because the band is measured on the *advance*, a click in the paragraph-spacing gutter belongs to the column on its left.
- **↑/↓ cannot change column.** ←/→ still walk the text buffer one character at a time, which visually runs *down* a column and then crosses the newline into the top of the next one — so left/right is how you move between columns. ↑/↓ go through the engine's caret geometry, and in vertical mode that geometry reports the caret's x as the **column's left edge regardless of row**, so the probe lands back in the same column one row up or down: they move a single character within the column and, at its first or last row, snap to the column's start or end rather than stepping sideways. Home / End and ⌘←/→ likewise run to the current column's top or bottom.
- **A path binding wins outright.** Path-bound text is composed by the Canvas2D path renderer, which never reads `vertical` (and for the layer being edited the engine's text sync is skipped altogether), so binding a vertical layer to a path lays it out horizontally along the curve. The toggle stays available and still reads as pressed while having no effect.
- **It round-trips.** `vertical` is written into `.lopsy` projects and read back — older files default to off — and clicking an existing vertical layer with the Text tool restores the toggle along with the rest of its settings.
- **The click target can fall short of the type.** The JS bounding box used to hit-test a text layer estimates a vertical layer as `column count × fontSize × lineHeight` wide by `longest column × (fontSize + letterSpacing)` tall, counting that longest column in **code points rather than shaped glyphs**. The width estimate multiplies by the column *width* while the engine lays columns out on the column *advance*, so with **Paragraph spacing above 0 the clickable box is narrower than the rendered columns by exactly `(columns − 1) × paragraphSpacing`** — three columns of 24 px type at paragraph spacing 20 reach x = 140.8 but stop being clickable at x = 100.8, so the whole last column is missed by clicks that visibly land on it.
...
- **Previews are served from one baked, offline blob.** `public/font-previews.bin` (~4.4 MB) concatenates a name-only WOFF2 subset for **1,934** of the catalog's Google families, and a generated index carries each family's byte offset and length. The blob is fetched **once**, on an idle callback after startup, and every row slices its own bytes out of it into a `document.fonts` FontFace registered under an alias, `<Family> Lopsy Preview` (`previewFontFamily`), never the real family name — a subset holding only the name's glyphs would otherwise stand in for the full font wherever the browser draws that family (#823, fixed in #856) — so after that single request the picker draws its entire dropdown with no network traffic at all, scrolling included. **Six** families failed to bake and fall back to the Google `css2` API's `text=` subset per row (fetched as CSS and injected as a `<style>` whose `@font-face` family is rewritten to the same alias, `renameCss2FontFamily`) (as do the two catalog entries that duplicate a bundled system face, Inter and JetBrains Mono). This replaced a per-row `<img>` pointing at a third-party PNG CDN whose GitHub repo had been deleted, which left every Google row showing a broken image (#729 / #730).
- **Until a row's face resolves it renders in its category's CSS generic**, via `font-family: '<Family> Lopsy Preview', '<Family>', <category>` — the preview alias first, then the real family (which resolves once that family's stylesheet has been loaded by picking it). Only `sans-serif`, `serif`, and `monospace` are real CSS generics — the **827 display and handwriting families** name a keyword the browser does not recognise, so for that instant they fall back to the default UI font rather than to anything stylistically close.
...
- **Local fonts** (Chromium desktop only — Chrome and Edge; Firefox and Safari have no Local Font Access API, and there the feature is simply absent). Once the editor is open — after the New Document modal, never at startup — the app calls `queryLocalFonts()`, which on first use raises Chrome's *see fonts installed on your device* permission prompt. Installed families are listed under a **Local** header at the top of the dropdown, one row per family, sorted alphabetically, each rendered in its own face. That takes an explicit load: Chromium hides installed families from CSS `font-family` lookup (an anti-fingerprinting measure), so naming the family alone would draw every row in its category fallback. When a Local row scrolls into view the picker reads that family's bytes through the Font Access API and registers every face with `document.fonts` (`loadLocalFontToDom`, one in-flight load per family, remembered for the session); a row whose bytes fail to load stays in the fallback face. The weight dropdown is derived from the OS style names: Thin/Hairline → 100, Ultra/ExtraLight → 200, Light → 300, Medium → 500, Semi/Demi Bold → 600, Bold → 700, Extra/UltraBold → 800, Heavy/Black → 900, ExtraBlack → 1000, Hiragino-style `W3`/`W6` → 300/600, anything else (Regular, Book, Roman, Plain) → 400; Italic/Oblique/Inclined faces set the italic flag. macOS-internal families whose name starts with `.` are dropped.
...
- **Picking a local family** reads every face of the family via `FontData.blob()` (raw SFNT bytes — macOS `true`-tagged TrueType, CFF OpenType and TTC members all parse) and loads them all into the engine's font database, so a later weight or italic switch needs no further I/O; the text re-renders off the Inter fallback once the bytes land, the same anchored refresh a Google download gets. The store keeps only the `FontData` handles, not the bytes — re-reading a local blob is cheap. Concurrent picks of the same family share one in-flight promise, and because reading the blobs is asynchronous the loader re-fetches the current engine afterwards before pushing bytes into it — **File → New** destroys and recreates the engine, so the one captured at the start of the read may already be gone.
- **When there is nothing to show but the API exists** — permission not yet requested, the prompt dismissed, or the query answered with an empty list because access was denied — the dropdown offers a **Load local fonts** button that re-queries from a click. The API is only exposed in secure contexts (https or localhost); current Chromium does not require a user gesture for the call itself. The automatic query fires **once**: it is gated on the document being ready *and* on the store still being `idle`, so after any outcome other than success the button is the only way back in.
- **A tab that was never in front is the one case that heals itself.** Chrome rejects `queryLocalFonts()` outright while the tab is hidden — open the editor in a background tab and the call fails with *Page needs to be visible* rather than prompting. The store notices that the failure happened while `document.visibilityState === 'hidden'` and registers a one-shot `visibilitychange` listener, so the query re-runs by itself the first time you actually look at the tab. Every other failure is terminal until the button is clicked.
- **Loading is on demand**: picking a Google family injects its `css2` stylesheet for DOM previews *and* fetches the binary for the selected weight into the WASM engine's font database so the canvas renders the real face. The binary comes from the exact TTF path baked into the generated catalog (jsDelivr → `google/fonts` repo), falling back to the css2 API's latin-subset WOFF2 (decoded in the engine) and finally to the bundled Inter if every fetch fails. Fetched binaries are cached per family+weight, so switching back is instant.
  - **The WOFF2 fallback decodes correctly (#841, fixed in #856).** The engine's decoder shifted every known-table tag from index 14 on (so `gasp` / `kern` / `GSUB` / `GPOS` / `fvar` / `gvar` were mislabelled), misread the transformed `glyf` header and triplet encoding, copied transformed `hmtx` bytes verbatim, and wrote table offsets in a different order from the table data — so a css2-fallback family could fail to load and render in Inter. `woff2.rs` now follows the spec, inverts the `hmtx` transform, and rejects any other transformed table instead of emitting garbage.
  - **A binary counts as loaded only when the engine has a face for it.** `loadFontBinaryToEngine` loads through `loadFontDataForFamily`, which also registers the face under the **catalog** name (css2 variable subsets are named after their default instance, e.g. `Montserrat Thin`), then checks `isFontLoaded`. Bytes the engine rejects are neither cached nor reported as loaded, so a later request fetches again instead of the failure sticking for the session. A cached binary is re-registered only into a *new* engine (after File → New) rather than on every font apply, which used to stack duplicate faces, and the local-font loader skips a face the engine rejects and keeps loading the rest of the family.
  - **Stylesheet faces are loaded eagerly.** When the css2 stylesheet arrives, `loadGoogleFont` calls `document.fonts.load` for each requested weight, so the faces download even if nothing has drawn with them yet.
- **When a binary lands, every text layer in that family is refreshed (#836, fixed in #838) and stays put (#888, fixed in #891).** Until #838 only the editing preview or the one layer whose font had just been changed was refreshed, so a layer committed while its binary was still downloading stayed on the Inter fallback. The refresh fires when a family or weight picked in the Text options finishes loading (`applyTextFontFamily` / `applyTextWeight`). `refreshTextAfterFontLoad` re-renders the live-editing preview, or else the committed layer whose font was changed (at its tracked anchor), and then every *other* committed text layer whose family matches; path-bound siblings only have their path-text cache invalidated. Those siblings have no tracked anchor, and the refresh used to treat their stored `x`/`y` as the anchor — but for centered or right-aligned area text `x`/`y` is already the texture's top-left (anchor plus the alignment offset), so the offset was counted twice and unrelated centered layers jumped right whenever a download finished. `refreshCommittedTextLayerFont` now recovers each sibling's anchor from its still-cached fallback layout *before* dropping that cache, then re-renders there. Caveat: if a text edit is live when the binary lands, only the editing preview is refreshed — the function returns before the sibling pass.
...
- **Committing renames the layer after its text.** Every commit — a new layer or a re-edit of an existing one — sets the layer's name to its content with whitespace runs collapsed to single spaces, trimmed, and cut to the first **16** characters (`textLayerNameFromContent`), so the Layers panel shows `Hello world` rather than the auto-generated `Text 3` the layer was created with. Because it runs on every commit, **a name typed into the Layers panel is overwritten the next time the text is edited**. A newly placed text layer also becomes the only selected layer, so a multi-selection made beforehand does not carry over to sit beside it.
- **Escape**: cancel the edit. If the layer was newly created in this editing session, it is removed entirely; otherwise the layer gets its prior text, glyphs, and position back — as it does when a re-edit is committed with all its text deleted. The live preview renders each keystroke straight into the layer's GPU texture, so the texture is copied when editing begins; cancel blits that copy back, and a commit pushes it as the `Text` history entry's *before* state, so undoing a text edit reverts the rendered glyphs as well as the text (#813).
- **Caret movement**: ←/→ move one character; **⌥←/→** jump by word (Unicode letter/number/underscore runs); **⌘←/→** and **Home/End** jump to line start/end; **↑/↓** move a line at a time, preserving a *goal column* across short lines via engine-provided caret geometry (on the first/last line they snap to the line boundary). All of this is described for horizontal text; in a **vertical** layer the same keys behave differently — see [Vertical text](#text-tool).
...
- **Backspace / Delete** remove the selection if there is one, otherwise the character before/after the caret (a whole code point — an emoji's surrogate pair is never split).
- **IME, emoji and symbol input**: while editing, a hidden focused textarea docked under the caret acts as the input sink, so text that never arrives as a printable keydown — Japanese/Chinese/Korean IME commits, the emoji picker and Character Viewer, dictation — lands in the buffer. An in-progress IME composition is previewed inline in the layer and replaced by the committed text; cancelling it restores the buffer. Keys that belong to a composition (`isComposing`, `Process`, keyCode 229) are left to the IME; every other key is still handled by the editor's keydown path (#803).
...
- **Move the selection outline**: with a rectangular or elliptical marquee tool active, press-drag from *inside* an existing selection to translate the selection mask itself — the marching-ants outline moves while the underlying pixels stay put (any active floating selection is dropped first). Arrow keys nudge the same marquee bounds. **Right after a Move-tool selection drag this branch catches presses it shouldn't**: the float commit runs first and swaps in the whole layer's alpha, so a press anywhere on the layer's pixels counts as inside and drags that outline instead of drawing a new marquee (see [Move](#move)).
...
- **Selection → Path**: traces the selection mask with marching squares, simplifies the contour with Douglas-Peucker, and fits smooth cubic Bezier anchors using Catmull-Rom tangents. The result is added to the Paths panel as a new path. Disabled when nothing is selected.
  - **The traced curve no longer kinks at every anchor (#821, fixed in #838).** `catmullRomToAnchors` used to build each anchor's `handleIn` from the *next* anchor's tangent and park it beside the next anchor, so every segment arrived at its end point from the wrong direction. `handleIn` is now `curr − tangentOut`, the mirror of `handleOut` about the anchor, so the two handles are collinear and every interior anchor is smooth. The first and last anchors of an open path still get no handles.
...
- The float is dropped — baking the result into the layer texture — on
  **Escape**, **`⌘D`**, or **selecting a different layer** in the Layers panel.
  ...
  "Selecting a different layer" means *clicking its row*: Add Layer, Merge Down,
  and Delete Layer change the active layer without committing, and each leaves
  the float behind to misbehave (see [Move](#move)).
...
- The selection mask is not recomputed during the drag. It is rebuilt only
  ...
  click, a pending transform or not (see [Quick transforms](#quick-transforms));
  and **`Delete`** (#801 — #808 took it off this list, #839 put it back). The swap pushes no history row of its own, so no single undo
...
- **Flip Horizontal / Flip Vertical** (options bar, next to the mode buttons)
  ...
  - **Known defect — a pending scale or rotate is thrown away (#800; fixed in
    #808, reverted by #839).** With a shape-changing transform pending
    (`isShapeChangingTransform`: any rotation, scale, skew, or corner offset),
    `applyGpuTransform` keeps the live float, but `compositeFloatAffine` then
    renders the float's *original* lifted pixels through the bare flip matrix —
    the pending transform is not composed in — before the float is dropped and
    the selection rebuilt from the layer's alpha. The committed result is the
    flipped but **unrotated, unscaled** piece, under a *Transform* history row.
    The selection-branch Rotate 90° buttons share the function and behave the
    same way.
...
- **Multi-layer drag**: when several layers are multi-selected in the Layers panel, a drag moves **all of them together**. Each selected layer's starting position is captured at pointer-down, and every tick applies the same delta to each — so the group translates rigidly rather than each layer being dragged independently.
  - **Locked layers in the selection stay put.** They are filtered out when the sibling list is built, so a locked layer inside a multi-selection anchors while the rest move. That only holds while the *active* layer is unlocked: if the active layer is the locked one, the canvas pointer-down guard refuses the whole drag and `handleNudgeMove` returns before its sibling loop, so **nothing in the selection moves — by drag or by arrow key** (see *Locked* under [Layer Properties](#layer-properties)).
...
- Arrow key nudge — 1 px by default, **10 px with Shift held** (#776); when grid + snap-to-grid is enabled, each key press nudges by exactly one grid cell, with or without Shift (the grid step replaces both distances rather than multiplying the Shift one). Arrow keys also nudge the active marquee bounds when a selection tool is active — but the responding set is an explicit list of **five** tools (rectangular marquee, elliptical marquee, lasso, magnetic lasso, magic wand). **Quick Selection is not in it**, so arrow keys do not nudge a selection while that tool is active; switch to any other selection tool and the same selection nudges fine. Under every other tool the arrow keys fall through untouched.
  ...
  - **The history entry is pushed on key-down, before anything is known to move.** The nudge coalescer records one `Nudge` row per key-hold synchronously on the first key-down and only then hands the delta to the tool, so an arrow press that moves nothing still leaves a row and clears the redo stack — see the defect note under [History](#history). It is the opposite of the lazy **Move** entry described below.
  - **Known defect — after undoing a selection drag, the arrow keys stop working until the next drag.** Undo and redo drop the engine's float but leave the Move tool's own record of it in place, and `handleNudgeMove` checks only that record, never the engine (the pointer-down handler checks both, which is why dragging still works). Each arrow press then throws `No float base` from the composite call. Nothing moves, pixels or marquee, yet the `Nudge` row is still pushed first, so the undone step **can no longer be redone**. Verified live: marquee → Move-drag → `⌘Z` → `→` five times left the pixels and the selection untouched, logged five errors, and replaced the redo stack's *Move* with a *Nudge* row. A Move drag, Escape, the `⌘D` key, or a Layers-panel click clears the stale record. In the control, a fresh marquee then `→` five times moved the piece 5 px.
- Snap to grid
...
- **A drag with an active marquee leaves the float live after release**, just as a transform-handle drag does — a follow-up drag keeps moving the same pixels, and nothing is baked until Escape, the `⌘D` key, undo/redo, a click on another layer's row in the Layers panel, or a canvas press with another tool. Two consequences are easy to hit. The next canvas press with **any other tool** commits the float by re-selecting the **whole layer's** alpha, so the marquee grows to cover everything on the layer (see [Commit lifecycle](#commit-lifecycle)). And **`Delete`** in that state clears the entire layer (#801), while a **`⌘X`** is reverted by the next drag (both under [Clipboard](#clipboard)). Five more actions go wrong in that state, because they neither commit the float nor account for it:
  - **Known defect — edits made while the float is live are thrown away by the next drag.** The engine keeps the lifted pixels and the layer-with-a-hole as two separate textures, and every Move drag (and, through the same composite call, every arrow-key nudge) rewrites the layer from those two. Nothing outside the Move tool updates them. So an edit that writes the layer in between — Edit → **Fill**, a **filter**, `⌘X` — shows until the next drag or nudge and then disappears, while its history row stays on the stack. Verified live on a 100 × 80 black block: marquee → Move-drag 60 px → Edit → Fill with red (the 120 × 100 selection turned red, 12 000 px) → Move-drag 30 px left **no red at all** and the original black block, moved. Filter → Invert in the same state went the same way. The control was Fill *before* the first drag, then a drag, and that moved the red.
  - **Known defect — Select-menu changes don't reach the float.** `⌘A`, `⇧⌘I`, and the other selection commands replace the selection, but the Move tool keeps both the float it already has *and the marquee it floated with*. The next drag moves only the previously floated piece, and on release the selection snaps back to that old marquee at its new offset. Verified with two blocks on one layer: marquee the first → drag 60 px → `⌘A` (selection 400 × 300) → drag 30 px moved **only the first block**, and the selection came back as 120 × 100. The control pressed the `⌘D` key first and then did `⌘A` + drag: both blocks moved and the selection stayed 400 × 300.
  - **Known defect — Add Layer leaves the float attached to the old layer.** Only a click on a Layers-panel row runs the commit. **Add Layer** makes the new layer active without it, so with the selection still up, a Move drag on the new, empty layer moves the floated pixels **on the previous layer**. Verified: float on Background → Add Layer → a 30 px drag moved Background's block 30 px while the new layer stayed empty. The control pressed the `⌘D` key before Add Layer, and the same drag moved nothing.
  - **Known defect — if the float's own layer goes away, the Move tool breaks.** After **Merge Down** (`⌘E`) or the Layers panel's **Delete Layer**, the engine still aims the float at the removed layer. Every pointer-move of the next Move drag then throws `Layer texture not found`: no pixels move, but the marquee outline still follows the pointer and a *Move* row is pushed. Verified for both. The control ran Merge Down with no live float, then a marquee + drag, and the pixels moved normally. Undo, Escape, the `⌘D` key, or a Layers-panel click clears the state.
  - **Known defect — a Marquee press right after a Move drag drags the outline instead of starting a new selection.** The commit that every non-Move canvas press runs happens *before* the rectangular or elliptical marquee decides what the press means, and by then it has replaced the selection with the whole layer's alpha. The marquee's "press inside the selection?" test therefore sees a selection covering every non-transparent pixel, and on an opaque layer every press passes it. The drag then **moves that whole-layer outline** (see *Move the selection outline* under [Selection Operations](#selection-operations)) instead of drawing a rectangle. Verified: 120 × 100 marquee → Move-drag → a Marquee drag from (20, 20) to (80, 80) left a **400 × 300** selection at (60, 60). The control skipped the Move drag, and the same Marquee drag made the expected 60 × 60 at (20, 20). A second Marquee drag works normally, because the float is gone by then.
- **Dragging inside a transformed selection moves just the transformed pixels (#786, fixed in #791).** After rotating or scaling a marquee with the transform handles, a drag inside the box reuses the live GPU float — its bounds become the selection's post-transform bounding box, and the transform's original mask is carried over to rebuild the selection on release. Until #791 that drag committed the transform and re-selected the **whole layer's** alpha first, so it floated every opaque pixel on the layer and dragged unselected pixels along with the piece. The old commit-and-reselect path now runs only when the engine holds no float to reuse.
  - **Known defect — the drag drops the pending rotation and scale (#806; fixed in #808, reverted by #839).** The press clears `persistentTransformRef`, and every move re-renders the reused float with the plain `compositeFloat(engine, dx, dy)`, which draws the *base* float — the originally lifted, untransformed pixels — at the drag offset. A rotated or scaled piece therefore snaps back to 0° / 100 % the moment it is dragged and is committed that way, while the marching ants keep following the transformed bounding box.
...
- **Delete/Backspace (Clear Selection) does not touch the internal clipboard (#870, fixed in #886).** Clearing a selection used to run through `clipboardCut`, which re-copies the *cleared* layer's pixels into `engine.clipboard_texture` as a side effect before wiping them — overwriting whatever `⌘C`/`⌘X` had staged there, on any layer, not just the one being cleared. The next `⌘V` then failed the internal-clipboard pixel/dimension match and fell back to the external-paste route (lands at 0,0, auto-selects, switches to Move). Delete now calls a dedicated `clearSelectedPixels` GPU op that clears without touching the clipboard texture, so an intervening Delete no longer breaks a pending paste-in-place.
...
- **Clicks that cannot fill record nothing (#811, fixed in #838).** On the normal-mode route, `handleFillDown` now returns *before* `pushHistory('Bucket Fill')` when the click lands outside the canvas or on a pixel the active selection fully excludes. Neither case leaves an empty *Bucket Fill* row, clears the redo stack, or adds a recent color. An outside-the-canvas click used to be worse than empty: on an empty layer it took the whole-layer solid-fill fast path and flooded the entire layer. The Quick Mask and layer-mask routes run earlier and are unchanged.
...
- **Stops**: 2 - 16 color stops with position (0-1), edited in the **Gradient modal** (opened by clicking either the gradient swatch or the "Advanced…" button in the options bar — both go to the same place). Click an empty spot on the handle row to insert a stop there, drag a handle to reposition it, and select a handle to drive a full ColorPicker (SV square + hue strip + alpha bar, with no hex, RGB, or HSV text fields; see [Color Picker](#color-picker-shared-component)) with a percent position readout. Delete is gated at the 2-stop minimum; the 16-stop cap matches the GPU uniform limit. Stops are re-sorted by position on every edit. `Escape` or an overlay click closes the modal.
...
### Drop Shadow
Defaults: disabled, black at 100% color alpha, offset 4 / 4, blur 8, spread 0, opacity 0.75.
...
**Layer effects inside an adjusted group are adjusted with it (#796, fixed in #856).** A group with adjustment nodes composites its children into a group scratch texture, but the children's glow / shadow / stroke passes always wrote to the main composite — underneath the scratch that was then blended over them — so adding any adjustment to a group (Vignette, Saturation & Vibrance, …) hid every effect inside it. Effect passes now render into the same target as their layer, so they land in the scratch, receive the group's adjustments, and are part of the cached pre-adjustment scratch (#663). The export composite (`compositeForExport` / `compositeForExportU16`, now sharing `composite_layers_for_export`) routes effects the same way and blends the group with its opacity, blend mode and mask as the live canvas does; and because the scratch holds straight alpha it is no longer blended as premultiplied, which had over-brightened partially transparent child pixels such as a glow's falloff.
...
- **Adding, removing, toggling and reordering nodes are undoable (added in #856).** Each pushes a metadata history entry — *Add Adjustment*, *Remove Adjustment*, *Toggle Adjustment*, and *Reorder Adjustments* (one per completed drag-drop) — so Add Adjustment can be undone; before, none of the four recorded anything.
...
**Nested groups each keep their own stack (#857, fixed in #866).** An adjusted group inside another adjusted group — the root group included — is finalized with its own adjustments first, and its output is then blended into the parent's scratch, so the parent's nodes apply on top. The compositor tracks groups with a stack (`group_stack` in `composite()` and the export path) and a per-group pre-adjustment cache that it bypasses for groups taking part in nesting. `engine-sync`'s `flattenGroupDescendants` stops descending at a nested group that is itself routed for adjustments or a mask, so every layer maps to exactly one immediate adjusted parent. Before, a single active-group slot let one group's pass claim the other's leaves, so once any root-group adjustment was active every nested stack stopped rendering (a nested Hue / Saturation +120 that turned a red rect green reverted to red).
...
- **A filter grows the layer to at least document size.** `ensure_layer_full_size` runs first, so a layer texture smaller than the scratch never makes the filter sample the scratch's unwritten region. The margin it adds is transparent, and for an edit-in-place filter (blur, adjust, sharpen) nothing looks different.
  - **The scratch grows with the layer (#862, fixed in #883).** `scratch_texture_a` / `_b` used to be sized only at `set_document_size`, but the filter passes bind their FBOs with the viewport at the layer's *current* texture size — which `ensure_layer_covers` grows past the document when content hangs off a canvas edge. Rendering then clipped against the smaller attachment, so Motion Blur (or any `apply_filter` / `apply_separable_blur` filter) on a block moved half off the bottom edge shifted its pixels instead of only softening them (in the repro the top edge jumped from y ≈ 240 to ≈ 267). `ensure_layer_covers` now calls `ensure_scratch_covers`, which reallocates both scratch textures and re-attaches their FBOs whenever the layer's new size exceeds them; they only grow.
- **The "generators" now generate onto transparency (#766, fixed in #770).** Clouds, Smoke, Fibers, and Voronoi end their shaders with alpha **1.0** (`fragColor = vec4(vec3(pattern), 1.0)`) rather than passing the source pixel's alpha through, so the document-sized expansion above is what makes them cover the canvas: run on the empty `Layer 1` of a new document, Clouds now fills the whole layer with an opaque pattern. Before the fix they wrote RGB only, so on transparency they did nothing and on a partly painted layer the pattern appeared only inside existing alpha. With a selection active the pattern is still confined to it by the usual `mix(original, filtered, mask)`, so a feathered marquee fades opaque clouds into the untouched layer. **Add Noise stays alpha-preserving on purpose** (`vec4(clamp(c.rgb, 0, 1), c.a)`): it perturbs existing pixels and leaves transparent ones transparent.
...
- **The store is told about the expansion (#771, fixed in #772).** The expansion re-origins the *engine's* copy of the layer descriptor — `ensure_layer_covers` writes `layer.x = min_x`, `layer.y = min_y` and the new dimensions — and `sync-layer-after-full-size.ts` exists to copy that into the Zustand store, because otherwise the next `syncLayers` push (any visibility, opacity, blend-mode, rename, or effect change) clobbers the engine's expanded bounds with the stale JS ones. The paint tools, the gradient, Smudge, the bucket, Edit → Fill, and the brush pre-warm always called it, and the [Shape tool](#shape-tool) keeps its own copy; until #772 **nothing driven from the menu bar did**, so a filter on a moved (content-cropped) layer permanently relocated it by its own `(x, y)` at the next descriptor change — verified at the time as a stroke's bounding box jumping from `(125, 73) – (234, 123)` to `(250, 146) – (359, 196)` after `Filter → Invert` and one visibility toggle. #772 adds `syncLayerAfterFullSize` to every commit path in `filter-actions.ts` (direct apply, preview begin, and preview commit) and to Pattern Fill, Color LUT, Liquify, Mesh Warp, and Tilt-Shift Blur.
...
- **Halftone**: dot size 2 - 32 px, density 0.25 - 3 (default 1.0 — scales dot coverage/frequency relative to the cell grid), angle 0 - 180 degrees, softness 0 - 4. Cell luminance is measured on **Rec. 601** weights, unlike every other tonal filter — see the luminance-weighting split under [Color](#color).
  - **Softness 0 is a hard edge, not a solid fill (#878, fixed in #894).** Softness is the half-width, in pixels, of the `smoothstep` ramp around each dot's radius. At 0 both `smoothstep` edges were equal, which GLSL ES leaves undefined, and on the headless test GPU it evaluated to 1 past the dot radius — light cells rendered almost fully opaque (~99 % on the light half of a gradient) instead of as tiny dots. The shader now floors the edge separation at `1e-4` (`max(u_contrast, 1e-4)`), which looks identical to a hard step and keeps the slider continuous down to 0.
...
- **Kaleidoscope**: segments 2 - 32, rotation 0 - 360 degrees (mirrors the image into a radial wedge pattern around the center of the layer's texture, which is resized to full document size first)
  - **Mirrored copies keep their true distance on non-square layers (#889, fixed in #892).** The polar fold used to run in raw UV space, where one unit of x and one of y cover different pixel counts, so on a non-square layer the copies stretched along the long axis — on a 400 × 200 document, a dot 50 px above center mirrored its horizontal copies 100 px out. The shader now scales into pixel space with a `u_size` uniform (the texture's `w × h`, read after `ensure_layer_full_size`) before computing radius and angle, and divides back out before sampling, so every copy lands at the same pixel radius.
...
- **Bloom**: threshold 0 - 100%, soft knee 0 - 100%, radius 1 - 64 px, intensity 0 - 200% (extracts bright areas above the threshold, applies Gaussian blur, and additively blends the glow back onto the original image — creates cinematic light bloom and soft glow effects around highlights)
  - **The glow is a centred Gaussian (#828, fixed in #838).** The shared `gaussian_blur.glsl` reads `u_weights[0]` as the centre tap and `u_weights[i]` as the weight for offsets ±i, but Bloom uploaded its `2r + 1`-tap kernel from the first element — the far tail. The centre therefore got the smallest weight and the peak landed at ±`radius`, so the glow came out as an offset echo of the highlights instead of a soft halo. `filter_bloom` now uploads the kernel starting at its centre tap (`kernel[radius + i]`).
...
- **Lens Distortion**: strength -100 to +100 (negative = pincushion, positive = barrel), zoom 50 - 200%, chromatic fringing 0 - 100% (applies barrel or pincushion radial distortion with optional per-channel color separation at edges, simulating real camera lens effects)
  - **The Strength track is not symmetric, and it is the only filter slider in the app that isn't.** The slider is normalized to −1…1 and handed to `lens_distortion.glsl`, which then boosts *only* the positive half: `if (k > 0.0) k *= 1.0 + 2.0 * k`. Pincushion stays linear (−100 → k = −1) while barrel goes quadratic (+25 → 0.375, **+50 → 1.0**, +75 → 1.875, **+100 → 3.0**). So full barrel bends **three times** as hard as full pincushion, and +50 already matches the magnitude of the entire pincushion half — the two sides of centre are nowhere near mirror images. (No other filter shader remaps one half of a signed parameter differently; Brightness/Contrast, Hue/Saturation and Lens Distortion are the only signed sliders, and the first two are linear throughout.)
  - **Barrel throws the frame's edges away; pincushion never does.** The shader inverts the mapping — it distorts the *sample* coordinate rather than the destination — so barrel pushes samples outward, and anything landing outside `[0, 1]` is written as `vec4(0.0)`, fully transparent. Pincushion pulls samples inward, so it can never leave the texture. At **zoom 100 the corner is already out of bounds at strength 1**, and at the dialog's own defaults (strength **50**, zoom **100**) not just the corners but the edge midpoints come back transparent — `(1, 0.5)` samples at `1.125`.
  - **That is what Zoom is for, and its default does not cover the strength default.** Zoom divides the sample offset, pulling the frame back inside; at strength 50 it takes about **zoom 130** to bring the corner back into bounds. Since the default pairing is 50 / 100, opening Lens Distortion and pressing Apply without touching anything punches transparent wedges into the edges of the layer. Strength 0 is the identity, but it sits at the **middle** of the track, not at the end — this is the only filter whose default is a strong effect on a slider whose no-op position is elsewhere on the same track.
...
- **Liquify** (Filter menu → "Liquify…" or `⌘⇧X`): opens a floating, modal-style session that paints into a per-pixel displacement map sampled by the GPU on each frame. Apply commits the warp to a new history snapshot; Cancel discards the displacement map.
  ...
  - **Push Forward pushes along the drag (#861, fixed in #884).** `liquify_warp.glsl` resamples backwards (`srcUv = v_uv + disp`), so moving a pixel along +drag needs `disp = −drag`. The push dab used to accumulate `+drag`, pulling in whatever lay *ahead* of the stroke: dragging down across a shape's bottom edge bit a notch upward instead of pushing a bulge out. It now subtracts. Bloat and Pinch already used the correct sign, and Twirl never reads the drag.
  - **Brush Size**: 4 - 500 px
  ...
  - **Undo reverts an applied warp (#816, fixed in #838).** A live session paints each dab straight into the layer texture, so the *Liquify* snapshot that Apply pushed already held the warped pixels, and `⌘Z` restored the warp. `applyLiquify` now blits the pre-session copy back (`restoreFilterPreview`) and reconciles bounds *before* `pushHistory('Liquify')`, then re-renders the final warp on top.
...
- **Pattern Fill**: tiles a user-defined pattern across the active layer. Reached from **Edit → Fill with Pattern…** (it is documented here with the other render filters because it shares the generic filter-dialog machinery, but it has no Filter-menu entry of its own).
  ...
  - **Tiles on a never-painted layer (#848, fixed in #864).** `filter_pattern_fill` read `u_layerSize` from the layer's texture *before* `apply_filter`'s own `ensure_layer_full_size` expanded it, so on the lazy 1 × 1 placeholder every fresh layer starts with, the shader got `(1, 1)` and every fragment sampled the pattern's top-left texel — a solid block instead of a tile. It now calls `ensure_layer_full_size` itself before reading the size, the same fix `fill_with_color` got in #770.
...
- **Visible**: on/off
- **Locked**: on/off, toggled from the lock button at the far right of the layer row. Locking is a **canvas-input guard only**. A single check at the top of the canvas pointer-down handler covers every tool at once, so no brush, eraser, shape, gradient, fill, stamp, or transform grab lands on a locked layer; arrow-key nudging is guarded separately, the brush pre-warm skips locked layers, and a locked **text** layer is dropped from the text hit-test so it cannot be clicked into for editing. Everything that does not originate as a canvas gesture ignores the flag — **filters, Merge Down, Flatten Image, Delete Layer, the Delete key's clear-selection, opacity/blend-mode/visibility edits, effects, and color-mode conversion all apply to a locked layer without complaint**. The one exception is **Edit → Fill**, which since #770 returns silently (no fill, no toast, no history row) when the active layer is locked. The engine is told about the flag but never consults it. Toggling the lock pushes **no history entry** (the store treats it as ephemeral UI), yet the state *is* written into `.lopsy` projects — so a lock survives save/reopen but cannot be undone.
  - **The check is on the *active* layer, not on what the tool would write — so it also switches off every tool that never touches that layer's pixels.** The lock test is the first statement of `handleToolDown`, ahead of the symmetry-center Cmd+click, the Liquify / Tilt-Shift / Mesh Warp session guards, and the whole tool dispatch. The only canvas presses that get past it are the ones the outer pointer handler consumes first: Space- or middle-button panning, two-finger pinch, and ruler clicks (adding or removing a guide, the guide-color corner — and only while guides are shown; see [Guides](#guides)). So with a locked layer active the **Eyedropper** samples nothing, no **marquee, lasso, Magic Wand, or Quick Selection** press makes — or click-deselects — a selection, **Crop** cannot be dragged, the **Text** tool can neither place a new layer nor click into a *different*, unlocked text layer, the **Pen** adds no anchors, and Cmd+click stops moving the symmetry center. Nothing signals why; the press simply does nothing. Verified live: the same Eyedropper click and marquee drag that did nothing over a locked layer sampled white and made a 683 × 590 selection once it was unlocked.
  - **Sessions opened from outside the canvas start anyway, then cannot be steered on it.** Filter → Liquify…, Filter → Tilt-Shift Blur, and the Move tool's Mesh Warp button never look at the flag, but their canvas guards run *behind* the lock check. On a locked layer Liquify strokes are refused, the Tilt-Shift band and handles do not drag — although its options-bar Blur Radius and **Apply** still work, so the default-positioned blur can be committed straight onto the locked layer — and Mesh Warp handles do not move.
  - **A locked active layer freezes a whole multi-selection**, drag and arrow key alike — see [Move](#move).
  - **Arrow-key selection nudges ignore it.** With a selection tool active the arrow keys go to `nudgeSelection`, which has no lock check, so a selection over a locked layer still moves by key (verified: bounds `x` 384 → 385) while every pointer press on that same selection is refused. On the Move tool the key press is refused but still records an empty `Nudge` history row — see [History](#history).
- **Position**: x, y
...
- **Mask**: grayscale mask with an `enabled` flag (see the caveat under Layer Operations — nothing in the UI can currently flip it). Mask painting with the brush, eraser, pencil, and gradient runs on the GPU mask texture with no per-frame CPU→GPU upload — **the bucket is the exception, and it is a CPU flood fill** (see [Editing a Layer Mask](#editing-a-layer-mask), which also covers why a mask dab is *not* as cheap as a layer dab). For the four GPU tools the GPU mask texture is the working copy: pointer-down makes sure the engine holds the mask (`uploadLayerMaskIfChanged`, #791: it uploads `layer.mask.data` only if the engine has never seen that array — re-uploading it would at best echo ~17 MB / ~80 ms at 4K and at worst overwrite newer GPU content with lagging bytes), every dab in the stroke renders GPU-side, and the texture is read back into the layer model *lazily*. **Since #780 undo does not depend on that readback.** A pixel history entry snapshots each layer mask the same way it snapshots layer pixels — a GPU→GPU blit into the engine's snapshot store (`snapshotMaskGpu`), restored on undo/redo with `restoreMaskFromGpuSnapshot` — and a mask nothing has written since the previous entry shares that entry's handle. `readMaskTexture` is a synchronous `glReadPixels` that stalls until every draw call the compositor still has queued has drained (15–20 s on a full-canvas 4K stroke before #759, 2.3 s on SwiftShader with the #760 queue in place), so it is queued through `mask-read-queue.ts` and fires only after a run of quiet animation frames (3 s cap). Until #780 the *next* mask stroke, fill, or gradient drained that queue on pointer-down, and `pushHistory` drained it again, so a back-to-back stroke paid the whole stall at its own start; neither does now. The JS bytes therefore lag the GPU, and the few readers that need them current read the mask back themselves at a non-interactive moment (`materializeMaskData` in `mask-data-sync.ts`): Save Project, Export PSD, Duplicate Layer, and Remove Mask. The mask thumbnail simply updates when the lazy read lands. Undo seeds the upload gate with the restored document's mask array, so bytes that still lag the restored GPU mask are never uploaded over it, and it queues a fresh readback when they do lag. (Quick Mask has no equivalent snapshot, which is why its strokes are **not** undoable — see Quick Mask Mode.) **Only the mask crosses the bridge.** Until #733 a mask stroke also expanded and cached the layer's *own* RGBA at pointer-down — the guard that skips that work for Quick Mask never named layer-mask mode — so every stroke dragged four bytes a pixel of layer data nobody would read across, on top of the mask's one; and until #734 the stroke-end readback was stored as a freshly allocated array whose new object identity tripped `syncLayers`' reference-equality gate, echoing the same bytes straight back to the GPU on the next frame. The readback's own array is now seeded as the tracked reference, so the upload gate recognizes them as already resident.
...
- **Only those five know the mask is there.** Dodge / Burn, Sponge, Clone
  ...
  the identical hole. (Before #895 the skipped setup was also where a
  spurious *Eraser* history entry came from, so these tools pushed a
  single correctly-labelled entry only in a mask mode. Since #895 they
  push a single entry everywhere — see
...
- **The mask bucket is not the Fill tool.** It reads Tolerance and Contiguous and
  ...
  mask bucket still moves the entire mask across the bridge inside the click:
  the engine reads the whole texture back, runs a 4-way CPU flood fill in Rust,
  and re-uploads the whole texture. Since #780 (in #856) the other two crossings
  are off the click: JS uploads `layer.mask.data` only when the engine has never
  seen that array (`uploadLayerMaskIfChanged`), and the filled result reaches the
  layer model through the lazy mask readback (`scheduleMaskDataRefresh`) instead
...
- **That readback moves one byte per pixel, not four — but only since #745.** A
  mask is conceptually 8-bit grayscale, yet it lives in an RGBA texture, and the
  stroke-end read used to fetch it as RGBA8 and then walk the buffer on the CPU
  taking every fourth byte. Both halves of that were expensive: a 4K mask is
  **64 MB** of transfer to recover **16 MB** of signal, and the extract was a
  16-million-iteration strided loop that misses cache on every step. The read now
  blits into an **R8** staging texture and calls `readPixels` with
  `RED / UNSIGNED_BYTE`, so the bridge carries 16 MB and the CPU loop is gone
  entirely. Measured on the 4K probe, this removed a **26.5-second** post-stroke
  stall — and with it a scaling curve that had been running roughly 10× worse for
  each 4× in area, i.e. badly super-linear, because the strided extract's cache
  behavior degraded as the buffer outgrew the caches. The same helper
...
- **The crop no longer drags the layer's mask with it (#850, fixed in #868).** A raster layer is always at `x = 0, y = 0` and document size while active, which is when its mask is created, so a raster mask is anchored at the document origin. `blend.glsl` used to sample the mask at the layer's `x` / `y`, so once the idle crop moved those off `(0, 0)` the mask appeared shifted — usually off its own edge, hiding nothing. The mask is now sampled at `u_maskOffset` from `mask_doc_offset()`: always `(0, 0)` for raster and group layers, and the layer's own position for text and shape layers, which are never cropped. Merge Down uses the same offset.
...
- **New Layer** (`⇧⌘N`, menu-only accelerator — see Single-Key Shortcuts note): appends a blank raster layer above the active one. Refused in **Indexed** color mode (the document is a single flat surface) with an info toast pointing at converting back to RGB. The same guard covers **Group Layers** and **Duplicate Layer**, but *not* text layers — clicking with the Text tool in an Indexed document still adds one.
  - **The new layer lands in its parent's `children[]` at its stacking slot (#824, fixed in #856).** Add Layer, New Group, text layers and all three paste routes put the new id into `layerOrder` just above the active layer but *appended* it to the parent group's `children[]`, so whenever the active layer was not the group's top child the two orders disagreed. `addToGroup` now takes the updated `layerOrder` and inserts the id before the first child that stacks above it.
- **Duplicate Layer** (`⌘J`, menu-only accelerator): clones the active layer and its GPU texture, inserting the copy directly above the original in the same parent group. The copy is **offset +10 px right and +10 px down — it is not cloned in place.** That offset is clamped so the copy's far edge never crosses the canvas edge the original was inside: the shift shrinks to whatever room remains, and drops to **0** for a layer already as wide (or as tall) as the canvas. Duplicating a **group** copies the group and every descendant recursively, remapping the child references, and applies one offset — computed from the group row itself — to the whole subtree.
  - **The clamp measures a raster layer's content, not its texture (#835, fixed in #838).** A raster layer's store bounds can be the document-sized rect its texture was expanded to, which is as large as the canvas, so the clamp computed a 0 offset and the copy landed exactly on the original. `computeDuplicateLayer` now feeds the clamp the engine's `getLayerContentBounds` for raster layers, so the copy gets the full +10 / +10 unless the *content* is near the edge. The shift is still applied to the stored `x` / `y`.
  - **Known defect — the pre-duplicate selection survives (#804; fixed in #808, reverted by #839).** Neither path touches `selectedLayerIds`: the copy becomes active while the original stays selected, so a Move drag straight after `⌘J` treats the original as a multi-selected sibling and drags it along with the copy. Clicking the copy's row first selects it alone. (The Move tool's option-drag duplicate skips the sibling capture for exactly this reason — see [Move](#move).)
  - **Known defect — a duplicated group is interleaved with its source (#805; fixed in #808, reverted by #839).** The group path splices each copy into `layerOrder` directly after *its own* source, one id at a time, so copies and originals alternate instead of forming one block. The Layers panel and the engine both follow `layerOrder`, so the copy's children show up among the source group's and are composited interleaved. The copy is also appended to the end of its parent's `children[]` instead of at its stack slot — Duplicate Layer, on both the group and the single-layer path, is the one layer-creating action that still calls `addToGroup` without `layerOrder` since #856.
...
- **Group Layers** (`⌘G`, menu-only accelerator): wraps the currently-selected layers in a new group
  - **The stack is left looking exactly as it did (#784, fixed in #791).** The grouped layers keep their relative order, and the new group is inserted at the **topmost selected layer's original z-position** inside that layer's parent, so unrelated siblings above the selection stay above it. Until #791 the action stored the children in display (top → bottom) order where `children[]` and `layerOrder` expect bottom → top — silently **reversing** the grouped layers — and inserted the block just before the target group, which for the root group meant hoisting the new group to the very top of the document.
  - **Known defect — a selected group's children are left behind (#807; fixed in #808, reverted by #839).** The `layerOrder` block is built from the selected ids alone. So when the selection includes a group, the new group's `children[]` are right but that group's descendants stay at their old positions in `layerOrder`, and the panel and the engine, which both follow `layerOrder`, show them out of place and composite them below layers they should cover.
  - **A non-contiguous selection no longer splits `children[]` from `layerOrder` (#881, fixed in #893).** The `layerOrder` block was already placed at the topmost selected layer's slot (#784), but the new group was *appended* to its parent's `children[]`. Selecting, say, the bottom and middle of three siblings therefore hoisted the group above the untouched top sibling in `children[]`, while `layerOrder`, the Layers panel and the compositor kept it below — so everything that walks `children[]` (PSD export, `flattenGroupDescendants`, drop validation, `.lopsy` serialization) saw a different stack. `groupSelectedLayers` now passes the rebuilt `layerOrder` into `addToGroup`, which inserts the group before the first sibling ranked above it — the same pattern `addGroup` uses.
- **Merge Down** (`⌘E`): composites the active layer into the layer below (the only layer-menu accelerator actually wired to a key handler)
  - **Refused when the layer below is a group (#879, fixed in #896).** A group has no raster texture, so `mergeLayers` composited into nothing while `computeMergeDown` still dropped the active layer from `layerOrder`, silently destroying its pixels. `computeMergeDown` now returns early when the entry below the active layer in `layerOrder` is a group: no history row, nothing changes; the `⌘E` shortcut reaches the same guard. The Layer menu's **Merge Down** item is greyed out by a separate `canMergeDown()` check, which also disables it when the active layer is the bottom entry of `layerOrder` (e.g. a lone `Background`) or there is no active layer.
  - **The top layer's mask is applied (#851, fixed in #869).** `merge_layers` used to blend the active layer with `u_hasMask = 0`, so its masked-out pixels came back in the merged result. It now binds the top layer's mask when that mask is enabled, sampled at `mask_doc_offset` like the compositor. The **bottom** layer's mask is deliberately not baked: it survives on the merged layer and keeps masking it. **Known gap (code-read, not live-verified):** when the top layer also has enabled effects, the effect bake runs first through `composite_single_layer`, which blends with no mask, and the re-pushed descriptor carries `mask: null`, so `merge_layers` finds no enabled mask — a masked layer *with* effects still loses its mask on Merge Down.
  - **Text is rasterized by the merge (#859, fixed in #869).** If either layer is text, the surviving layer becomes a plain `raster` at `0, 0` and document size. It keeps only its id, name, visibility, lock, `clipToBelow`, and its own mask (the same whitelist as Rasterize Layer), with opacity 1, Normal blend, and default effects. It used to stay `type: 'text'` with the old string and doc-sized bounds, so the next Size or font edit re-typeset the stale text and threw the merged pixels away.
...
- **Rasterize Layer**: bakes a **text** layer's current visual into pixels in place, reading the engine's current x/y/w/h so the result lands at the visible position even after GPU texture expansion from upstream paint ops. The button appears in the Layers panel toolbar only while a text layer is active — there is currently no rasterize entry point for shape or group layers.
  - **Known defect — a Move-tool rotate or scale of a text layer does not stick (#798; fixed in #808, reverted by #839).** Committing a handle transform bakes the transformed pixels into the text layer's texture, but the layer stays `type: 'text'`: `clearPersistentTransform` — run by Escape, the `⌘D` key, and every Layers-panel row click — only drops the float and clears the JS pixel cache. The next Text-panel edit therefore re-renders the string from scratch and the transform is lost; Rasterize Layer first if the transform must survive. #808 made that routine convert the layer to raster on commit. As written it fired for any active text layer, transformed or not (#812), and #838 limited it to a pending transform — but #839 removed the conversion outright, so nothing rasterizes text implicitly now and #812 cannot occur.
...
- **Cmd/Ctrl+click a layer thumbnail**: loads that layer's alpha as a marquee selection — each pixel's alpha becomes the selection mask value (anything below 1 is dropped), so a soft edge yields a feathered selection rather than a hard one. Any live GPU float is committed first and the layer's JS pixel cache is cleared, so the selection reflects the finished pixels. It also **seeds a transform state** from the resulting bounds and schedules a prefloat, meaning the selection comes up ready to scale/rotate — **except on a text layer**, where the prefloat is skipped (#785, fixed in #791): the prefloat runs the engine's `ensure_layer_full_size`, which re-origins the layer to `(0, 0)` and breaks the anchor the text re-render relies on, so the next Text-tool click on empty canvas near the top-left was routed into that layer and snapped the text to the canvas corner. It is the same invariant #767 fixed for the Brush pre-warm; the marching-ants selection itself is unchanged.
  - **The prefloat mirrors the float's whole rect into the store (#802, #810; fixed in #856).** On a raster layer, floating runs the engine's `ensure_layer_full_size`, re-origining the texture to the document rect (union its content). `executePrefloat` used to copy only the new origin into `layer.x` / `layer.y`, leaving `width` / `height` at the cropped size — a phantom rectangle at the canvas corner that Snap to Layers pulled drags toward (#802) — and after ⌘D dropped the float the next full resync (any undo or redo) pushed the stale bounds back onto the document-sized texture and threw the content off by its own offset, usually off-canvas (#810; repro: 600 × 400 document, circle at 360..480 × 90..210 on an inactive Layer 1, store `360, 90, 120 × 120` vs engine `0, 0, 600 × 400`). It now calls `reconcileLayerBoundsWithEngine`, which copies the engine's placement — x/y, plus width/height for raster layers — into the store for whichever layer's thumbnail was clicked, active or not. After ⌘-click + `⌘D` the store and the texture both describe that expanded rect until the crop-on-leave cycle shrinks the layer back (see [Layer Texture Lifecycle](#layer-texture-lifecycle-crop-on-switch-expand-on-return)); `⌘D` itself only drops the float, with no crop. (#808 had briefly made deselect crop the active layer back to content; #839 reverted that.)

  **Group rows have no thumbnail** (they show a collapse chevron in that slot), so there is no way to load a group's combined alpha this way.
...
**Duplicate, Merge Down, Rasterize Layer Style and Flatten Image no longer resolve the whole document first (#746).** All four compute on the GPU — `duplicateLayerTexture`, `mergeLayers`, `rasterizeLayerEffects`, `compositeForExport` — and always did, but until #746 each was wrapped in a `resolveAllPixelData` before and a `syncPixelDataToGpu` after. That pair read **every layer in the document** back to the CPU and re-uploaded it, to feed a GPU path that never asked for the pixels: duplicating one layer in a 4K three-layer document moved **448 MB** across the bridge (three layers read, four written) and left a 64 MB `ImageData` cached JS-side per layer for nothing. The wrappers are gone; each action now invalidates only the bitmap cache of the layers it actually touched (`invalidateBitmapCache(activeId)`, plus `belowId` for Merge Down).

**Only Duplicate is genuinely pixel-free, though — the other three still round-trip their own result.** Each of these files carries a comment saying no JS-side pixel buffer is threaded through, and that is true of the *cache*, not of the *bridge*. `duplicateLayerTexture` really is texture-to-texture inside the engine and returns nothing. But `rasterizeLayerEffects` and `compositeForExport` **return the rendered bytes to JS**, which hands them straight back through `uploadLayerPixels` without looking at them — one full-canvas buffer out and the same buffer in, **128 MB per call at 4K**. So Rasterize Layer Style and Flatten Image each pay that round trip every time, and Merge Down pays it **once per layer that has enabled effects** (zero when neither does, twice when both do, on top of the GPU-only `mergeLayers` itself). What #746 removed was the document-wide resolve, not these; the remaining traffic is transient and scoped to the layers being changed, and never enters the pixel manager.

**Align is the one layer operation that still reads every layer back.** `resolveAllPixelData` now has exactly one caller left in the store — `alignLayer` — and it is a fair one: aligning by *content* bounds rather than by the layer rect means somebody has to scan alpha, and a group aligns by the combined bounds of its descendants. So Align keeps the whole-document readback the other four shed.
...
Dragging a row's grip starts a pointer-driven reorder; there is no HTML5 drag-and-drop and no keyboard equivalent. The dragged row fades to 40 % opacity. **Since #856 every drop resolves to a single tree slot — a parent group plus the sibling it lands directly under — and the same slot drives both the indicator and the drop** (`resolveGapDrop` in `useLayerDnd.ts`, applied by the `dropLayer` store action), so a layer always lands where the indicator says (#814, #824, fixed in #856).

- **Between rows** — the pointer's position within the row it is over picks a gap: above the row in the top half, below it in the bottom half. Nothing can be dropped above the root row.
- **Depth follows sideways pointer travel.** The drop depth starts at the dragged row's own depth and shifts one level per **16 px** of horizontal travel (`DROP_DEPTH_STEP_PX`, twice the 8 px row indent so jitter doesn't flip it), clamped to the depths the gap can hold: no shallower than the row below the gap, no deeper than the row above — or one deeper when that row is an **expanded** group, meaning "top of that group". A straight vertical drag therefore keeps a layer at its level (root rows stay at the root, group children stay in their group); drag left to outdent — including dragging a nested group out to the root — or right to indent. This settles the gap under a group's last child (#814): at the child's depth the layer goes to the bottom of the group, one level out it goes below the whole group.
- **The indicator shows that depth**: a **2 px accent line** on the neighbouring row's edge, indented `depth × 8 px`.
- **Into a group** — hovering the **middle half (25 %–75 %) of a group row** targets the group itself, drawn as a **2 px accent outline plus a tinted background**; the layer drops in as the group's **topmost** child. Offered only when legal (`canMoveToGroup` rejects dropping a group into itself or its own descendant); collapsed groups accept it too.
- **A group moves with its subtree as one contiguous block.** `computeDropLayer` lifts the dragged layer — plus, for a group, every descendant — out of `layerOrder`, reinserts the block directly below the target sibling's whole subtree (or directly below the parent's own entry for "top of group"), and rewrites `children[]` from that same order, so the tree and the stacking cannot disagree.
- **No-op drops show nothing and record nothing.** A slot equal to the layer's current one hides the indicator, and `computeDropLayer` returns nothing for an unchanged or invalid move, so **Reorder Layer** is pushed only when something actually moves. Dropping onto the row of the group a layer already sits in is a no-op only if it is already that group's top child; otherwise it moves the layer to the top.
- **Collapsed groups don't throw the drop off (#797; #808's fix was reverted by #839, #856's resolver fixes it).** The resolver works on the displayed rows, with the dragged row and its subtree removed first, so a collapsed group's hidden children are never candidates, and a gap under a collapsed group can only mean "below the group".
- **Paste no longer knocks the panel out of step (#824, fixed in #856).** The old resolver indexed `document.layers` while the reorder used `layerOrder`, and new layers were appended to `children[]` out of stacking order (see *New Layer* under [Layer Operations](#layer-operations)); both are gone.
...
- **Add Layer** and **New Group** — always present.
  - **New Group records no history.** `addGroup` inserts the group and makes it active with a bare `set`, never calling `pushHistory` or `pushHistoryMetadata` — unlike Add Layer and Group Layers, which push metadata snapshots. So the group is not an undo step of its own: it rides along with whatever edit came before it, and the first `⌘Z` afterwards reverts that edit **and** removes the group, because the state it restores was captured before the group existed. `⇧⌘Z` brings both back. Verified live: `Delete Layer` → New Group → `⌘Z` restored the deleted layer and the group vanished with no row of its own; redo returned the document to the post-delete state with the group in it.
...
- **Delete Layer** — pushed to the far right by a spacer. Deletes every selected non-root layer, and is disabled when nothing deletable is selected or `layers.length <= 1`.
  - **The next active layer is the deleted one's neighbour (#819, fixed in #838).** When the active layer goes, `computeRemoveLayer` activates the nearest non-group sibling in the same parent — first the one directly *below* it, then the one above — then the parent group itself (unless that is the root), then the nearest non-group layer below and then above in `layerOrder`. It used to jump to the **bottom-most** raster layer in the document (usually *Background*) wherever the deleted layer sat. The button runs this once per deleted layer, and the layer left active ends up the only selected one.
  - **That count includes the root group, so the last real layer can be deleted.** A document whose only layer is `Background` holds two entries in `layers` — `Background` and the root group — so the button stays enabled, and the store-side guard in `computeRemoveLayer` (the same `layers.length <= 1` test) lets the `Backspace` / `Delete` key through as well. The button only disables once the root group is all that is left. The document is then empty: the root group becomes the active layer, and every paint tool is refused on it with the *Groups can't be painted on* toast (see [Layer Types](#layer-types)) until Add Layer gives it something to hold. Verified live on a one-layer document: the button was enabled, one click removed `Background` under a `Delete Layers` row, and the button then went disabled with only the root group remaining; the `Backspace` key did the same under a `Delete Layer` row.
...
- **Known defect — `Delete` while a float is live clears the entire layer (#801; fixed in #808, reverted by #839).** Before clearing, the Delete handler checks for a live GPU float, and if there is one it calls `selectLayerAlpha` on the active layer — which drops the float and **replaces the selection with that whole layer's alpha**, not with the region that was floated. The clear then runs against the replacement. A float stays live after any Move-tool drag of a marquee selection, after any transform-handle drag, and while a thumbnail ⌘-click prefloat is up, until Escape, the `⌘D` key, undo/redo, or a click on another layer's row commits it (see [Commit lifecycle](#commit-lifecycle)). So *select, drag the piece somewhere, press Delete* is enough, and a selection loaded from *another* layer's thumbnail clears the whole active layer instead of that shape. Verified live before #808: an opaque 400 × 300 layer, a 100 × 80 marquee dragged 60 px with the Move tool, then `Delete` → **0 of 120 000 pixels left**, and the selection had become 400 × 300. The control — the same marquee and `Delete` without the drag — cleared only the rectangle. The `Clear Selection` row is pushed *after* the selection swap, so `⌘Z` brings the pixels back but not the marquee. **`⌘X` is not affected** — Cut calls the same engine clear but skips the float check.
- **Known defect — a Cut made while a float is live is undone by the next Move drag.** Cut clears the pixels from the layer texture, but the float texture still holds them, and the Move tool re-composites that float from its saved base on every drag. Verified live: marquee → Move-drag 60 px → `⌘X` (the 100 × 80 block cleared, clipboard filled) → a second Move drag of 40 px, after which the layer held **exactly** the pre-Cut content with the block moved the extra 40 px. The `Cut` row stays on the undo stack and the clipboard keeps its copy, so a paste now duplicates the block. Cut is not special here: Edit → Fill and the filters are discarded by the next drag in the same way (see [Move](#move)).
...
- **Fill** (Edit menu, `⇧F5` — menu-only accelerator, not wired to a global key): fills the current selection (or the entire layer if no selection) on the active layer with the **foreground color**, immediately. There is no dialog and no color/pattern choice — to fill with anything else, set the foreground color first or use Fill with Pattern. The fill runs on the GPU and honors the active selection mask.
  - **It expands the layer first (#765, fixed in #770).** `fill_with_color` now calls `ensure_layer_full_size` before filling and `fillSelection` reconciles the store with `syncLayerAfterFullSize` afterwards. Before the fix it filled whatever texture the layer had: on the lazy **1 × 1** placeholder every untouched layer starts with (so *New Document → Edit → Fill* on `Layer 1`) it wrote one pixel of stale scratch content, not even the foreground color, and on a moved, content-cropped layer it produced a squashed block instead of the filled rectangle.
  - **It silently refuses locked, group, and text layers.** A locked active layer returns with no fill, no toast, and no history row — the one menu writer that honors the lock — and a group or text layer gets the layer-type toast (see [Layer Types](#layer-types)).
...
samples it at `(docPos − u_maskOffset) / u_maskSize`, i.e. 1:1 document pixels
anchored at the document origin for raster and group layers (at the layer's own
`x` / `y` for text and shape layers — see `mask_doc_offset`, #868), and where that UV
falls outside `0…1` the mask
...
- **Image Size** shows a **Constrain proportions** checkbox (default on) and a
  ...
  **no resample-method choice**: `scale_texture` is always bilinear — since
  #856 computed in premultiplied space by the `blit_resample` shader
  (`samplePremulBilinear`: four `texelFetch` taps interpolated premultiplied,
  un-premultiplied on return) instead of by switching the texture to hardware
  `LINEAR`, which bled transparent texels' black RGB into anti-aliased edges
  (#815).
...
- **Cmd/Ctrl+scroll**: zoom in / out (factor `1.002^-deltaY`, clamped to 0.01× – 64×); plain scroll pans. The zoom is anchored to the **cursor**: `zoomAtPoint` rewrites the pan along with the zoom so the document point under the pointer stays put (#777). The wheel zoom is continuous — it does not step through the zoom levels below.
- **Trackpad pinch in Safari**: Safari reports a pinch as its proprietary `gesturestart` / `gesturechange` / `gestureend` events rather than the Ctrl+wheel Chrome and Firefox send, so the canvas listens for those too. The zoom scales from the value at gesture start by the event's cumulative `scale`, anchored at the gesture's starting point and panned as its center drifts; Ctrl+wheel events that arrive during a Safari pinch are ignored so the zoom is not applied twice.
- **Two-finger pinch** (touch): pinching anywhere in the viewport scales the zoom from the gesture's start distance (clamped 0.01× – 64×), keeping the document point under the starting midpoint pinned beneath the moving midpoint, so the pinch zooms about the fingers and pans with them. The gesture is picked up even when a finger lands on surrounding UI chrome rather than the canvas, and it cancels any in-flight tool stroke first so a pinch never leaves a stray mark.
- **Cmd/Ctrl + `=`** / **Cmd/Ctrl + `-`** (and View → Zoom In / Zoom Out): step to the next / previous **fixed zoom level** (#777) — 1, 2, 3, 4, 5, 6.25, 8.33, 12.5, 16.67, 25, 33.33, 50, 66.67, 100, 200, 300, 400, 500, 600, 700, 800, 1200, 1600, 2400, 3200, 4800, 6400 % — the Photoshop sequence, so repeated presses land on exact ratios instead of drifting through powers of 1.5. From an off-list zoom (after a wheel zoom or a fit) the first press goes to the nearest level in that direction; a 0.1 % tolerance counts a zoom that is within float noise of a level as already on it. The ends clamp at 1 % and 6400 %. These steps are anchored to the **viewport center**, not the cursor (only `viewport.zoom` changes).
...
- **Rendering at device resolution** (#778): the WebGL canvas and the overlay canvas are backed at `devicePixelRatio`, so on a 2× Retina display they draw at native resolution instead of being upscaled and blurred by the browser. Zoom and pan remain in CSS pixels, which means **100 % is one document pixel per CSS pixel** — on a 2× display, 50 % is the zoom at which one document pixel maps to one physical pixel. The engine canvas is re-sized every frame, not only on resize, because moving the window to a display with a different pixel ratio fires no resize event.
- **Crisp sampling in both directions** (`final_blit.glsl`, #778). **Above 100 %** each document pixel is drawn as a flat square, blended only across the one screen pixel where two texels meet — sharper than bilinear smearing, and even at fractional zooms like 130 % where nearest-neighbor would make pixels uneven sizes. **Below 100 %** each screen pixel averages a grid of bilinear taps spread across its footprint (`ceil(1/zoom)` per side, capped at 8 × 8), alpha-weighted so transparent texels do not darken the edges they average with, so hairlines and thin text do not alias or shimmer while panning.
...
- **Tick spacing follows the zoom.** The renderer takes `50 / zoom` document pixels as a target and rounds it **down** to the nearest entry in the 1-2-5 × power-of-ten sequence (`… 1, 2, 5, 10, 20, 50, 100 …`), with a floor of 1 px — so the labelled interval changes as you zoom and always lands on a round number instead of an arbitrary one. 100 % zoom labels every 50 px, 200 % every 20, 400 % every 10, 800 % every 5. Ticks are 6 px long and drawn only on the inner edge. **The rounding happens in the ruler's own unit** (see *Units* below): the target is converted first, so a millimetre ruler labels 10, 20, 50 mm and never the 28.35 px a 10 mm step is worth, and at 72 dpi and 100 % an inch ruler labels every ½ in. Inches below one use a physical ruler's binary series (½, ¼, ⅛, 1/16, …, halving without limit as you zoom in) instead of the decimal one. Ticks are placed by index (`i × step`) rather than by accumulating the step, so a fractional pixel interval never drifts along the ruler.
- **Labels carry no unit suffix.** In pixels they are rounded to integers; in the other units they are trimmed to at most four decimals (`0.5`, `0.0625`, `10`). The vertical ruler's labels are rotated a quarter turn. Positions outside the canvas are labelled with their real (negative or over-size) coordinates rather than being suppressed.
- **Units** — **View → Units**: Pixels / Points / Inches / Millimeters, default Pixels, a checkmark on the current one. Switches both rulers and the guide-placement hover readout. Physical units convert through the document's **dpi**: the New Document modal's *Resolution (DPI)* field is stored on the document (`document.dpi`, saved in the project file as `dpi`) — 72 for the pixel presets, 300 for US Letter and A4, or whatever was typed. Opened images and files saved before it was stored have none and are treated as **72 dpi**. Points are 1/72 in, so at 72 dpi a point ruler reads exactly like the pixel ruler. The unit is a session setting like the other View toggles (not persisted), and nothing outside the rulers follows it — the Info panel, status bar, and every dialog stay in pixels.
...
- Guide creation, removal, and the color picker all require **both** Show Rulers and Show Guides to be on.
  - **With guides hidden, a click on a visible ruler is a canvas click.** The pointer handler only *consumes* a ruler press inside the two branches that test `showGuides`; with Show Guides off neither runs, and the press falls straight through to the active tool's `handleToolDown` at whatever document position lies under the 20 px strip. The rulers are an overlay on the canvas, so when the document reaches under them — zoomed in, or panned against the top or left edge — the Brush paints there, a marquee starts there, and so on. The corner square falls through the same way. (With Show Rulers off there is no ruler to click, so the question does not arise.) Verified live on a 400 × 300 document at 300 % with the Brush active: with guides shown, a click on the top ruler added a vertical guide and changed no pixels; after View → Show Guides was unchecked, the same click one ruler-width further along painted a 10 px dab at document (221–230, 31–40) — under the ruler — and pushed a `Brush` row.
  - Clicking the canvas while the guide-color picker is open closes the picker **and** applies the tool: the close is a side effect on the way to `handleToolDown`, not a consumed press.
...
Arrow-key nudging is also **not Move-tool-only** — the same `gridSize`-or-1 step (10 with Shift, which the grid step overrides) drives the five selection tools (both marquees, both lassos, Magic Wand), where it nudges the selection rather than the layer.
...
The strip across the very top of the window — the first thing in the app shell, above the options bar — holding **nine menus in a fixed order: File, Edit, Image, Layer, Select, Filter, Path, View, Help**, with a **Tutorials** link and a `LOPSY` wordmark pushed to the far right (the shared `BrandLinks` component, #855). The link opens `/tutorials/` in a **new tab** (`noopener`) so an open document is not lost; the wordmark is decorative and `aria-hidden`, set in **Jersey 10**, a third self-hosted face alongside Inter, used only for this wordmark and the identical one the New Document modal parks in the same corner — the first-run New Document screen renders the same `BrandLinks`, so the Tutorials link is reachable before any document exists. It is the entry point for every command that has no tool of its own, and it owns the dialogs those commands open. For the item-by-item contents of each menu, see the feature sections they belong to; this section covers the bar itself.
...
- **Checkmarks sit in a fixed 16 px gutter** reserved for any item that declares a checked state, so a run of toggles stays aligned whether or not any of them is currently on. Only three places use them: the **View** menu's seven toggles (Show Rulers, Show Grid, Show Pixel Grid, Show Guides, Snap to Grid, Snap to Layers, Show Seamless Pattern), the **View → Units** submenu, and the **Image → Mode** submenu. Every other item renders flush with no gutter.
...
- **Almost nothing greys out.** Exactly five items in the entire bar carry a disabled state. Four are gated on an active selection: Edit → **Crop**, **Define Brush…**, and **Define Color Brush…**, plus Select → **Selection → Path**. The fifth, Layer → **Merge Down**, is gated on `canMergeDown()`, which requires a non-group layer directly below the active one (#896). Every other command stays enabled and no-ops internally when it cannot run, so Undo with an empty history looks identical to the working case.
...
- **The menus are rebuilt from scratch on every render**, each definition snapshotting the stores through `getState()` rather than subscribing to them. Opening a menu is itself a state change, so checkmarks and those five disabled flags are always evaluated at the moment the dropdown appears — but they are then frozen for as long as it stays open.

**Keyboard support is the gap.** The bar renders the full ARIA vocabulary — `role="menu"`, `role="menuitem"`, `aria-haspopup`, `aria-expanded` — but implements none of the matching keyboard interaction: there are no key handlers anywhere in the menu bar. Arrow keys do not move along the titles or down a dropdown, there is no typeahead, no roving tabindex, and **Escape does not close an open menu** — only a second click on the title, a click outside, or picking a command will. What does work is plain tabbing: the titles are ordinary buttons, so Tab reaches them and Enter opens a dropdown, after which Tab walks its rows, disabled ones included. Space, the other standard button key, does not: the global shortcut handler cancels every Space `keydown` outside an input before the button sees it (see the focus guard under [Single-Key Shortcuts](#single-key-shortcuts)). This is the opposite of the dock's tab strip, which does implement the full arrow / Home / End pattern (see [Tab Keyboard Navigation](#tab-keyboard-navigation)).
...
- **A clicked toolbar button gives its focus back (#817, fixed in #856).** After a pointer click on any button inside a `role="toolbar"` — this bar, the toolbox, the panel-visibility toolbar, and the Layers panel's action row (now `role="toolbar"`, `aria-label="Layer actions"`) — a document-level click listener blurs it (`toolbar-button-focus.ts`). Before, the button kept focus and the next Enter re-activated it: Move → Flip Horizontal, then Enter to "commit", flipped the content straight back, and every further Enter toggled it. Keyboard activation (`click.detail === 0`) keeps focus, so a Tab-reached button still works, and blurring after the click rather than preventing focus on mousedown leaves `NumberInput`'s commit-on-blur intact.
...
- **Typed, stepped, and reset values are recorded in history like a drag (#846, fixed in #867).** The value box's blur/Enter commit, its ↑ / ↓ step, and double-click reset now call the slider's `onDragStart` / `onCommit` around the change, as the pointer drag always did. Sliders that attach history to those callbacks therefore record a typed value as its own undo step: the Text options bar's Size, the Text panel's Size / Line height / Letter spacing / Paragraph spacing (`beginTextLayerHistory`), and the Layer Effects glow / shadow / stroke forms (`beginEffectsDrag`). Before, typing a new Size left no row, and `⌘Z` popped the original text commit. **Known defect:** `handleBlur` records unconditionally, so focusing one of those value boxes and leaving it unchanged still pushes a history row (open PR #913 targets it).
...
- **`Cmd/Ctrl+scroll`** — zoom in / out, anchored to the cursor; plain scroll pans. **Two-finger pinch** on touch devices, and trackpad pinch in Safari, zoom about the fingers and pan with them.
- **`Backspace` / `Delete`** (canvas focused) — when a marquee selection is active, clears the selected pixels on the active layer (GPU clear, undoable as "Clear Selection") — hard-edged across a feather, and **the whole layer** if a Move or transform float is still live (#801; both under [Clipboard](#clipboard)); when no selection is active, removes the active layer from the document — **including the document's last one** (see [Layers Panel Toolbar](#layers-panel-toolbar)). Suppressed while a text layer is being edited or any `<input>` / `<textarea>` has focus — but not while a dropdown or button does (see the focus guard below).
...
**The focus guard is the input's type (#817, fixed in #838).** Every key above — the tool letters, `X` / `D` / `Q`, the brackets, arrow nudges, `Backspace` / `Delete`, `Escape`, `Enter`, `Space`, and every `⌘` combo including undo — runs through one capture-phase `keydown` listener on `window`. Its only opt-out, outside text-layer editing, is a focused **text-entry** field: a `<textarea>` other than the Text tool's hidden input sink, or an `<input>` whose type is `text`, `search`, `url`, `email`, `password`, `tel`, or `number` (a missing type counts as `text`). While a text layer is being edited, a focused non-text input is blurred and the key goes to the text editor. There is no check for an open dialog, for the Layers panel, or for any other kind of focused control, which cuts both ways:

- **Sliders and checkboxes no longer swallow shortcuts (#817, fixed in #838).** The app's sliders are `<input type="range">`, and Chromium focuses a range input when it is clicked. Until #838 the guard tested the tag alone, so after adjusting a slider every global key — tool letters, `⌘Z` — was ignored until focus moved elsewhere. Now the keys reach the global handler with the slider still focused, which has effects of its own. With the Move tool or a selection tool active, **arrow keys nudge the layer or selection instead of stepping the slider**, because the capture-phase nudge calls `preventDefault`. `Backspace` / `Delete` runs the canvas clear, or deletes the active layer. The text boxes beside each slider and `NumberInput` are `type="text"` and still swallow everything. Separately, a document-level click listener (`toolbar-button-focus.ts`, #856) blurs any button inside a `role="toolbar"` after a *pointer* click, so the next `Enter` or `Space` no longer re-fires the last clicked button (see [Options Bar](#options-bar)).
- **A `<select>` swallows nothing.** Dropdowns are not in the guard, so a tool letter typed while one has focus switches tools — and the options-bar dropdown that had focus is unmounted with the old tool's options. Verified live: with the Dodge tool's **Mode** dropdown focused, `B` selected the Brush. The same holds for the Unit dropdowns in New Document, Canvas Size, and Image Size, where `Backspace` runs the canvas delete behind the dialog.
- **Buttons and tabs swallow nothing either** — which is how `Backspace` reaches the canvas from inside the Keyboard Shortcuts modal (see [Keyboard Shortcut Customization](#keyboard-shortcut-customization)), and why arrowing along a dock tab strip can nudge a layer (see [Tab Keyboard Navigation](#tab-keyboard-navigation)).
- **`Space` has its default action cancelled wherever it lands outside a text-entry field** — a focused slider or checkbox included, so Space cannot toggle a checkbox. The Space branch calls `preventDefault` before arming the pan, and it runs ahead of the focused element. A cancelled Space `keydown` is what keeps a browser from activating a focused button, so Space is not a working activation key for the app's buttons — the menu-bar titles included. (Verified: a Space `keydown` dispatched on the focused **File** title comes back `defaultPrevented`; an Enter `keydown` does not.)
...
### Help Menu
- **No Tutorials item any more (#855).** #825 put *Tutorials* here, where it was unreachable until a document was open; it now lives as a link beside the `LOPSY` wordmark in the menu bar and on the first-run New Document screen (see [Menu Bar](#menu-bar) and [Tutorials](#tutorials)). The Help menu holds just the two items below.
...
- **Arming a row does not plug the gap — the captured key reaches the canvas too.** The listening handler is registered on `window` in the capture phase and calls `preventDefault` + `stopPropagation`, but the global shortcut handler is *also* a capture-phase listener on `window`, and it was registered first, when the editor mounted. Listeners on the same target run in registration order, and `stopPropagation` does not stop the others on that target (only `stopImmediatePropagation` would), so by the time the modal sees the key the canvas has already acted on it — using the bindings as they stood **before** the rebind. Verified live: arming the Pencil row and pressing `X` bound Pencil to `X` (banner: *"X" is already used by Swap Colors*) **and** swapped the foreground and background colors; arming the Brush row and pressing `Backspace` bound Brush to `BACKSPACE` **and** deleted the active layer, pushing a `Delete Layer` row and emptying the redo stack. Pressing `Escape` to cancel an armed row likewise clears the canvas selection and any pending transform first.
...
**Four keys are accepted as bindings but can never trigger one: `Space`, `Enter`, `Backspace`, and `Delete`.** The global handler gives each of them its own branch — Space arms panning, Enter strokes a Path-tool draft, Backspace / Delete run the delete action — and every one of those branches `return`s *before* the single-key tool dispatch is reached, whatever the active tool. The modal still stores the binding (and releases the action's old letter), so the tool is left with no working key at all while the key keeps its built-in meaning. (`Escape` is the fifth special key, but it cannot be bound: pressed while a row is armed, it cancels.) Verified live: with Brush bound to `BACKSPACE` and the Move tool active, pressing `Backspace` left the tool on Move and deleted the active layer.
...
- **With the Move tool or a selection tool active, the arrow keys also nudge.** The strip's own key handler and the global arrow-key nudge both see the key: the global listener runs first, in the capture phase, and has no focus check beyond `<input>` / `<textarea>`, so its `preventDefault` does not stop the tab's React handler and neither handler knows about the other. Arrowing along a strip therefore switches panels **and** moves the active layer (Move) or the selection (both marquees, both lassos, Magic Wand) by 1 px — or a grid cell with Snap to Grid — pushing a `Nudge` row each time. Home / End are unaffected. Verified live: with Move active, `→` on the focused **Color** tab selected **Info** and shifted the Background layer's content 1 px right under a new `Nudge` entry; with the Brush active, `←` on the same strip switched back with no nudge and no history row.
...
**Paths belong to one document (#873, fixed in #877).** `createDocument` and `openImageAsDocument` now clear `paths` and `selectedPathId` and drop the Pen tool's in-progress draft (`clearPath()`), alongside the clipboard and guides. Before, File → New and Open Image kept the old document's paths — in this panel, in the Text tool's Path dropdown, and on the canvas overlay, at the old coordinates. A `.lopsy` load re-seeds `paths` from its manifest afterwards. The name counter above is untouched, so numbering still continues.
...
- **Stroke Path** opens the **Stroke Path modal** rather than stroking immediately: a **Width** number field pre-filled from the Path tool's current stroke width and auto-selected on open (clamped and rounded to **1 – 50 px** on confirm), and a read-only **Color** row showing the **foreground color** swatch — the color is not editable here. `Enter` or **Stroke** rasterizes onto the **active layer**; `Escape`, **Cancel**, or a click on the backdrop dismisses it.
  - **The stroke lands on the path on a content-cropped layer (#820, fixed in #838).** `rasterizePathToLayer` expands the target layer to the canvas (`getOrCreateLayerPixelData`), which rewrites its `x` / `y`. It used to translate the document-space anchors by the pre-expansion offset from a stale store snapshot, so on a layer cropped away from the canvas origin the stroke landed shifted by that old offset. It now re-reads the layer after the expansion.
...
- **Thumbnails**: **40 × 20**, produced on the **GPU**. `readChannelThumbnail` / `readLayerThumbnail` blit the active layer's texture down into a small RGBA8 texture and read back only that, returning an 8-byte header (`width`, `height` as u32 LE) followed by RGBA pixels; the panel unpacks it into `ImageData` and letterboxes it into the 40 × 20 canvas when the aspect doesn't match. The CPU per-pixel extraction loop this replaced moved ~67 MB **per channel** per update on a 4K layer (#683). Reads are retried up to **10** times while the engine is still warming up — once per queue flush since #747, not once per animation frame — then give up and leave the tile blank.
...
- Thumbnails re-render whenever the **active** layer's pixel-data version increments — painting on any other layer leaves them alone. That version bumps once per *committed* edit rather than per input event: a brush stroke bumps at stroke end, and since #732 a **shape or gradient drag bumps only on pointer-up**. Both keep updating the canvas live off the GPU preview path, so mid-drag these tiles — along with the Layers-panel thumbnail and the histogram behind Curves / Levels, the two other surfaces that read back off this version — still show the pre-drag pixels. That is deliberate: firing these readbacks per pointer-move accounted for the great majority of a 4K shape drag's wall clock. **Since #747 all three read the same way.** #743 moved the Layers-panel thumbnail off the synchronous path onto `requestThumbnailRead`, a coalescing queue that flushes every pending read in one idle tick; #747 routed these Channels tiles and the histogram onto it too, so nothing reads a thumbnail straight off a `useEffect` any more. The bytes were never the problem — 24 × 24 × 4 is 2,304 of them — but a synchronous `glReadPixels` forces a pipeline flush that waits on every draw call the compositor still has in flight, and reacting to a pixel-version bump landed that stall on the frame right after the pen came up. This panel was the worst offender: its five rows fired **five** of those flushes per stroke-end, one per tile. Batching means one stall per tick instead of one per dirty read, and it lands while the browser is idle.
  - **The queue's dedup key is caller-supplied**, because a per-layer id is not unique here — five tiles share one layer. All five Channels rows key on `` `${layerId}:${channel}` `` — the composite row included, as `` `<id>:rgb` `` — so a later request for the same tile replaces the earlier one while the four siblings stay queued. The Layers-panel thumbnail keys on the bare layer id, which is why it never collides with the composite tile for the same layer. A flush snapshots and clears the map before invoking the readers, so a reader that re-queues itself on a null result lands in the *next* tick rather than the current one.
  - **The flush is an idle callback with a 200 ms timeout**, falling back to a plain 200 ms `setTimeout` on browsers without `requestIdleCallback`. Retries are therefore counted in queue ticks, not animation frames — **10** for the Channels tiles and the Layers thumbnail, **8** for the histogram — so a texture that is slow to warm up can take appreciably longer to appear than it did on the old per-frame retry. (The Navigator's composite-dirty signal listens to the same notification, but it suppresses ticks during any pointer gesture anyway, so its minimap is unchanged either way.)
...
A dockable typography panel for styling the active/selected committed text layer, covering everything in the text options bar **except the vertical toggle** (it was a strict superset until #752 added that one options-bar-only control). Grouped into four sections:
...
- **Decoration**: **underline (U)** and **strikethrough (S)** toggles. There is no vertical toggle here — that one exists only in the options bar (see [Vertical text](#text-tool)).
...
- **Caveat — this intercept is global, not brush-only.** The check runs near the top of the canvas pointer-down handler — after only the lock test and, for pixel-writing tools, the layer-type guard (so on a group or text layer the Brush's Cmd+click gets the layer-type toast instead) — before any tool dispatch, and it is not scoped to the paint tools. So while *any* symmetry axis is enabled, a Cmd/Meta+click is swallowed by the symmetry-center move for **every** tool — which suppresses the other documented Cmd/Meta gestures (shape and marquee 1:1 lock, gradient 15° snap, transform snap, Path anchor convert, the Cmd+shift 15° snap on straight-line brush/pencil/eraser strokes, and the Cmd half of the Clone Stamp / Healing source-set). Alt/Option still sets the stamp and healing source. Turn symmetry off to get those gestures back.
...
- **No text entry (#799; added in #808, reverted by #839).** The picker is canvases only — no hex field and no numeric R / G / B or H / S / V inputs — so on every surface that uses it alone (the Gradient modal, Gradient Map stops, the shape fill/stroke popover, the guide-color picker) a color cannot be typed. The Color panel is the exception, through its own hex field and sliders rendered below the picker (see [Hex field and sliders](#hex-field-and-sliders)).
...
- **Undo** (`⌘Z`) / **Redo** (`⇧⌘Z`) from the Edit menu or keyboard
  - **The keyboard shortcuts are synchronous again (#790).** #764 briefly routed `⌘Z` / `⇧⌘Z` through `scheduleUndoRedo`, a coalescer that counted presses and applied them once per `requestAnimationFrame` as a single `undoBy(N)` / `redoBy(N)`. That left the history stack unchanged immediately after the keypress, stranded batches whenever the browser throttled animation frames, and broke 51 undo/redo e2e tests across Chromium and Firefox. #790 removed the coalescer: the key handler calls `undo()` / `redo()` directly, exactly as the Edit-menu items do, so every press — including each auto-repeat of a held `⌘Z` — is one full restore applied on the same tick. The batched `undoBy(N)` / `redoBy(N)` primitives stayed and now back only the History-panel jump.
  - **Batched steps are equivalent to single ones.** `undoBy(N)` restores the target snapshot once and re-syncs once, and pushes onto the redo stack the same N entries N consecutive undos would have, so redo afterwards walks back through every intermediate state; `undo()` and `redo()` are now just `undoBy(1)` / `redoBy(1)`.
...
- **Layer masks are snapshotted the same way (#780).** Each pixel snapshot also holds a GPU snapshot handle per layer mask (`snapshotMaskGpu`, same store and handle space as layer snapshots), plus a flag recording whether that snapshot's own `layer.mask.data` still lagged the GPU when it was taken. A mask that nothing has written since the previous pixel snapshot — every wasm-bridge entry that writes a mask texture marks it in `mask-gpu-dirty.ts` — reuses that snapshot's handle; undo and redo mark every mask as written, mirroring `dirtyLayerIds`. Mask handles leak exactly like layer handles (see the defect below); the Move tool's discarded pre-float releases the fresh mask handles it took.
...
- **The entry an undo or redo pushes re-checks each layer's bounds (#833, fixed in #856).** Undo and redo build the "current state" entry they push onto the opposite stack from the just-restored snapshot's handles. But an undo that changes the active layer lets the render loop crop or expand layers outside history, so a layer restored as a cropped 200 × 200 texture could be document-sized by the next redo; that redo recorded the cropped handle with the expanded `0, 0, 600 × 400` bounds, and the undo after it drew the content at the layer origin. `snapshotLiveLayers` now reuses a restored handle only when the layer's position and raster size still match the ones it was captured with — the same test an ordinary push applies — and re-snapshots the rest from the live texture.
...
- **Metadata-only snapshots** skip the texture copy entirely and record just the model. They back **Add Layer**, **Add Text Layer**, **Group Layers**, **Toggle Visibility**, **Reorder Layer**, **Change Opacity**, **Change Blend Mode**, **Add / Remove / Toggle Mask**, the four adjustment-node edits (**Add Adjustment**, **Remove Adjustment**, **Toggle Adjustment**, **Reorder Adjustments**, since #856), every layer-effect edit (`Edit Drop Shadow`, `Enable Stroke`, …), and every Path-tool edit (**Add Path**, **Move Path Anchor**, **Straighten Path Anchor**, **Edit Path Handle**, **Split Path Segment**). Undoing one of them never touches the GPU at all: it restores the document model, the selection, and the paths, and leaves every layer texture exactly as it is.
  - **Restoring one keeps every layer's current geometry (#783, fixed in #791).** A metadata restore takes each layer's `x`, `y`, `width`, and `height` from the *current* document and only the other fields — blend mode, opacity, effects, mask, visibility, order, the active layer — from the snapshot. The texture is never restored on this path, and between the push and the undo `transitionActiveLayer` crops the outgoing layer and expands the incoming one on every layer switch, so a snapshot's descriptor routinely described a texture that no longer existed: restoring it verbatim teleported the content by exactly its own `(x, y)` — to `(2x, 2y)` after one undo and back to `(0, 0)` after a redo.
  - **A metadata step taken right after a mask stroke is safe to undo (#782).** A layer-mask stroke's readback is deferred for up to 3 s (see [Layer Properties](#layer-properties)), so a visibility toggle, Add Layer, reorder, or any other metadata step inside that window snapshots a mask array older than the GPU texture. Before #791 undoing that step wrote the stale mask back over the painted texture, erasing the stroke; #791 made `pushHistoryMetadata` drain the readback queue first, paying the stroke's synchronous readback on the metadata step. **Since #780 there is no drain:** undoing a metadata entry keeps the live GPU mask, seeds the upload gate with the restored array so it is never uploaded, and queues a lazy readback when that array differs from the live one.
...
- **Restoring is more than a texture swap.** Every undo and redo step first drops an active float and cancels a scheduled prefloat — a stale float keeps its expanded dimensions in the engine's layer descriptor and would misplace the restored texture (issue #706) — then restores each layer texture from the snapshot, resets the tracked engine state, clears every cached JS pixel buffer, and re-pushes all layer descriptors. **The reset keeps the mask and selection content references (#781, fixed in #791).** `resetTrackedState` used to wipe `maskDataRefs` too, so the next `syncLayers` saw no tracked ref for any mask and re-uploaded every one — measured at ~50 MB per `⌘Z` on a 4K document with three masks, whether or not the undone step touched a mask. Undo and redo now pass `preserveContentRefs`, which carries `maskDataRefs`, `masksOnEngine` and the selection-mask ref across the reset. Since #780 a mask restored from its GPU snapshot has its restored array seeded as the tracked ref, so it is never re-uploaded even though that array may lag the texture; only a mask with no usable handle (for instance one re-added by the undo) is uploaded from its array. Engine re-creation and project load reset without it, since stale refs against a fresh GPU would silently skip real uploads. Undo additionally finalizes a stroke still pending when it fires; redo does not.
- **Undo and redo re-seat a visible transform box (#871, fixed in #885).** The handle overlay (`uiStore.transform`) lives outside the history stack, so undoing a Move used to revert the pixels and selection but leave the box — and its scale / rotate pivot — where the Move had drawn it, and the next handle drag scaled the reverted content about the wrong pivot. `undoBy` / `redoBy` now end in `syncTransformAfterHistoryRestore`: if an overlay is showing, it is rebuilt at identity (`createTransformState`, keeping the mode) on the restored selection's bounds, or cleared if the restored selection is inactive. No overlay is created where none was showing.
...
- **Known defect — an arrow key that moves nothing still records a `Nudge`, and wipes the redo stack doing it.** The keyboard-nudge coalescer (`nudge-coalesce.ts`) calls `pushHistory('Nudge')` synchronously on the first key-down of a key-hold and only afterwards runs the tool's nudge, which is free to do nothing — and `pushHistory` has no no-op check, so the push also clears the redo stack and raises the dirty flag. Two everyday states hit it: **a selection tool (Rectangular / Elliptical Marquee, Lasso, Magnetic Lasso, Magic Wand) with no selection**, where `nudgeSelection` returns at once, and **the Move tool on a locked active layer**, where `handleNudgeMove` returns at its lock check. Each key-hold then adds an empty `Nudge` row, and whatever had been undone is gone. Verified live: Add Layer → `⌘Z` → `→` with the Rectangular Marquee and no selection left the stack at `New Document, Nudge` with an empty redo stack, and `⇧⌘Z` brought nothing back. It is the keyboard twin of the bare-click *Move* rows that #721 removed from pointer drags by pushing lazily (see [Move](#move)). A third state is easy to reach by accident: right after `⌘Z` undoes a Move-tool selection drag, the Move tool's nudge throws instead of returning, and leaves the same empty row and emptied redo stack behind (also under [Move](#move)).
- **Known defect — the same push-before-bail shape reaches one more tool.** `pushHistory` / `pushHistoryMetadata` is called at the *top* of several handlers, ahead of the guards that decide whether there is any work to do, and every guard that then returns leaves an empty row behind and an emptied redo stack. Besides `Nudge`, one is reachable from an ordinary gesture: a [path anchor or handle click that never becomes a drag](#path--pen-tool) (*Move Path Anchor*, *Edit Path Handle*). A bucket-fill click outside an active selection was a second until #838 moved the push below that check (#811). A census of all 85 push sites in the app found no others a user can trigger — the remaining returns that follow a push are either `.map` callbacks rather than bails, or they re-test a condition the caller already checked *before* pushing (`cropCanvas` re-checks its own zero-area rectangle, `convertColorMode` its own same-mode case), so they cannot fire.
- **Known defect — New Group is invisible to the stack.** (Until #770 the disagreement ran both ways: a Brush, Pencil, or Eraser stroke with a group active recorded a row for a stroke that could never be seen. Paint tools are now refused on groups before anything is pushed — see [Layer Types](#layer-types).) **New Group** changes the document and records no row at all, so the next undo takes the group away together with the edit before it (see [Layers Panel Toolbar](#layers-panel-toolbar)).
...
- The row matching the current state is highlighted in the accent color. Clicking any row jumps to it with **one** batched `undoBy(N)` / `redoBy(N)` call (#761): a single restore of the target snapshot and a single engine re-sync, however many rows are crossed. Before #764 the jump ran `undo` or `redo` once per row, each a full restore that re-uploaded every layer mask, so jumping thirty rows cost thirty of them. The jump is synchronous, like the keyboard shortcuts and the Edit-menu items.
...
- **Resolution** (`dpi`, #853): set only by the New Document dialog and saved in `.lopsy` files; absent (treated as 72) on opened images and older projects. It drives nothing but the rulers' physical units — see [Rulers](#rulers).
...
A static, search-oriented tutorials site served from the same origin as the editor at **`/tutorials/`** (#825). It is separate from the app: pages are plain HTML generated at build time, with **no JavaScript** on them, no backend, and no database. The editor links to it from the **Tutorials** link beside the `LOPSY` wordmark (menu bar and first-run New Document screen, #855 — Help → Tutorials was removed); each page links back with an **Open Lopsy** button in the header and an *Open the editor* footer link.

- **Content is Markdown in `tutorials/<slug>/index.md`** next to its screenshots; the folder name is the URL slug, and folders starting with `_` (the `_template/` starter) are ignored. Frontmatter carries `title`, `published` / `updated` dates, a `description`, an optional `level` (Beginner, Intermediate, or Advanced, case-insensitive), an optional `duration` in whole minutes, `tags`, `related` slugs, a `cover` image with its `coverAlt`, an optional `finished` image with its required `finishedAlt` (#906), and `draft`. `published` may carry an optional UTC time (`2026-09-25 18:41`, #855) used **only for ordering** so same-day tutorials list newest first; pages still show just the date. Each `## ` heading is **one step with exactly one image** and a short blurb. Only a small Markdown subset renders (bold, italic, inline code, links, `[[Key]]` keycaps, bullet and numbered lists, `> **Tip:**` callouts); anything else comes out as plain text.
- **Generated pages**: `/tutorials/` (the list, newest first), one page per tutorial with breadcrumbs, the **finished image** directly under the title and date line (#906 — a plain eager-loaded figure with no step number or text; the `finished` frontmatter image if set, otherwise the last step's image), the steps, a call to action, and up to **three** related tutorials (the `related` list first, then the ones sharing the most tags), plus `/sitemap.xml` listing every tutorial with its `updated` date (`robots.txt` points at it). Every page carries a canonical URL, Open Graph and Twitter-card tags, and schema.org JSON-LD — `HowTo` with one `HowToStep` per step and `BreadcrumbList` on tutorial pages, `CollectionPage` / `ItemList` on the list page. Image width and height are read from the files and written into the markup so pages do not shift as images load.
- **Build rules** (`scripts/vite-plugin-tutorials.ts`, logic in `src/site/tutorials/`): under `npm run dev` pages render on request, **include drafts**, and live-reload on save; `npm run build` drops drafts, minifies the CSS, and **fails** on content errors (a missing title or description, bad dates or `updated` earlier than `published`, a step without exactly one image or without alt text or text, images that are missing, unreadable, or outside the tutorial folder, unknown `related` slugs, a bad `level` or `duration`, a non-kebab-case folder name), while only **warning** when a title runs past 60 characters or a description falls outside 50 – 160, and when a file in a tutorial folder is not referenced by any step (it is not published).
- **Header**: the `LOPSY` wordmark in Jersey 10, matching the editor's own logo (#830; enlarged to 40 px in #855), plus *Tutorials* and *Open Lopsy* links.
- **Published as of this audit (14)**, newest first: *Make a Stencil Street Art Billboard* (#905, Intermediate, 60 min), *Design a Drippy Skate Snack Shack Menu* (#899, Intermediate, 75 min), *Design a Baroque Restaurant Menu* (#890, Advanced, 90 min), *Design a Screen-Print Style Restaurant Menu* (#882, Intermediate, 60 min), *Design a Surrealist Cinema Logo* (#875, Intermediate, 45 min), *Make a Liquid Chrome Text Billboard* (#863, Intermediate, 50 min), *Design a Skate-Style Restaurant Menu* (#847, Intermediate, 60 min), *Design a Folk Art Zine Cover* (#843, Intermediate, 60 min), *Design a Constructivist Zine Cover in Lopsy* (#837, Intermediate, 60 min), *Draw an Etching-Style Lighthouse Illustration* (#854, Advanced, 90 min — dated with no time, so it sorts as midnight and lands below the timed 09-25 entries), *Design a Swiss Style Exhibition Poster* (#834, Intermediate, 50 min), *Design a Propaganda Poster Style Party Invitation* (#832, Intermediate, 60 min), *Make a Neon Glow Text Effect* (Beginner, 5 min), and *Make a Vaporwave Sunset Billboard* (#829, Intermediate, 45 min).
...
- **The New Document modal's size controls.** Six **preset** tiles head the dialog — **Web 1080p** (1920 × 1080), **Web 720p** (1280 × 720), **Instagram Post** (1080 × 1080), **US Letter** (8.5 × 11 in), **A4** (8.27 × 11.69 in), and **4K UHD** (3840 × 2160) — and it opens on Web 1080p. Width and Height sit beside a **Unit** dropdown offering only **Pixels** and **Inches**; choosing Inches reveals a **Resolution (DPI)** field (default 72; the two print presets set 300), and switching units converts the typed values at that DPI — pixels → inches rounded to two decimals, inches → pixels rounded to whole pixels. Editing Width, Height, Unit, or DPI un-highlights the preset. On Create the size is converted to pixels and clamped to **1 – 16,384** on each side. The first-run instance, shown before any document exists, is the whole app and cannot be dismissed — no Cancel button, no Escape; the File → New instance adds both.
  - **The DPI is stored on the document — but only the rulers read it (#853).** `onCreateDocument` now passes the DPI through to `createDocument`, which writes it to `document.dpi` (72 for the pixel presets, 300 for US Letter and A4, or whatever was typed), and `.lopsy` project files save and restore it as `dpi`. Its only consumer is the ruler / guide-readout **View → Units** conversion (see [Rulers](#rulers)); it is not written to any export (no PNG `pHYs`, no JPEG JFIF density, no PSD resolution block), and opened images and pre-#853 project files carry none and are treated as 72. US Letter at 300 DPI still produces a **2550 × 3300 px** document. The DPI fields in [Canvas Size and Image Size](#the-two-dialogs), which offer the identical Pixels / Inches pair, **do not read it** — both still start at 72 — so create a US Letter document at 300 DPI and open Canvas Size in inches and it reads **35.42 × 45.83 in**, the pixel size divided by 72, while an inch ruler on the same document reads 8.5 × 11.
...
- **Not stored in the file at all**: the current selection, the undo history, quick-mask state, reference images, tool settings, brush presets, patterns, the foreground / background colors and the recent-colors strip, and the viewport. Loading always finishes with **fit-to-view**, so zoom and pan are not restored — a project saved at 400 % reopens fitted to the window.
...
- **The preview composites once per opening** (#762, shipped in #764). Opening the dialog renders the document composite, reads it back, and downscales it to a ≤ 220 px thumbnail a single time; every later format click or Quality change re-encodes only that cached thumbnail with `canvas.toBlob` (~3 ms), instead of re-compositing the whole document (~500 ms and 64 MB at 4K) as it used to. The preview therefore reflects the document as it was when the dialog opened, which is the only state it can be in while the modal is up.
