# Font-preview blob license

`public/font-previews.bin` contains name-only WOFF2 subsets of the Google-source families in
`src/utils/font-catalog.ts`, baked once at build time by
`scripts/generate-font-previews.ts`. The blob is used by the FontPicker so
each row renders its family name in its own face without a runtime request
per font.

Every family it contains is a Google Fonts release. Google Fonts are shipped
under one of the OFL 1.1, Apache 2.0, or Ubuntu Font License 1.0 — the
per-family license is listed under each family's directory in
[google/fonts](https://github.com/google/fonts). The great majority are
OFL 1.1; the full SIL Open Font License text is reproduced below and covers
distribution of the subsets in this blob.

Nothing in this repository asserts ownership of the font data — the fonts
remain the property of their respective foundries and designers.
