...
export const createDocumentSlice: SliceCreator<DocumentSlice> = (set, get) => ({
  ...
  groupSelectedLayers: () => {
    ...
    // Remove selected layers from their current parents, then add new group.
    // Must remove before adding the group so removeFromParentGroup doesn't
    // also strip children from the newly created group.
